...
To ensure that the ScholarMaster platform operates with absolute privacy guarantees, continuous edge performance, and institutional policy compliance, the system requirements are partitioned into Functional Requirements (FR) and Non-Functional Requirements (NFR).

\subsection{Functional Requirements (FR)}
\begin{enumerate}
    \item \textbf{FR-01 (Anonymous Pose Feature Extraction):} The system shall extract spatiotemporal pose skeletons ($17$ keypoint coordinates via YOLOv8-pose) without writing raw image frames or biometric facial pixels to persistent storage.
    \item \textbf{FR-02 (Open-Set Identity Verification):} The system shall support multi-identity enrollment and open-set identification scaling up to 100,000 enrolled profiles using 512-dimensional facial embeddings (InsightFace + FAISS HNSW graph).
    \item \textbf{FR-03 (Spatiotemporal Schedule Compliance):} The system shall execute real-time truancy and anomaly detection by matching localized student observations against institutional timetables using the Spatiotemporal Constraint Satisfaction Framework (ST-CSF).
    \item \textbf{FR-04 (Teleportation Detection Heuristic):} The system shall identify and filter tracking anomalies by computing bounding box center velocity, flagging physical impossibilities ($v_i > v_{max}$) as false tracking noise.
    \item \textbf{FR-05 (Non-Semantic Acoustic Anomaly Detection):} The system shall process ambient audio using Fast Fourier Transform (FFT) spectral features (Centroid, ZCR, Flux) to detect acoustic distress without transcribing spoken dialogue.
    \item \textbf{FR-06 (Cryptographic Audit Logging):} The system shall serialize all attendance events and compliance violations into a tamper-evident, append-only Merkle tree ledger to ensure non-repudiation.
    \item \textbf{FR-07 (Role-Based Access Control):} The system shall enforce a 7-tier Role-Based Access Control (RBAC) matrix governing permissions for Students, Faculty, HODs, Security, and System Administrators.
    \item \textbf{FR-08 (Federated Model Updating):} The system shall aggregate model weight gradients across edge nodes using Federated Averaging (FedAvg) while local raw data remains strictly confined to the origin node.
\end{enumerate}

\subsection{Non-Functional Requirements (NFR)}
\begin{enumerate}
    \item \textbf{NFR-01 (Real-Time Latency Floor):} The complete sensing-to-inference execution pipeline shall complete processing within a strict $33\text{ms}$ frame window ($\ge 30\text{ FPS}$).
    \item \textbf{NFR-02 (Volatile Memory Lifetime & TTL):} Raw video buffers shall remain in RAM for a maximum Time-to-Live ($\text{TTL} \le 33\text{ms}$), and ambient audio buffers for $\text{TTL} \le 3\text{s}$, after which explicit zeroization is enforced.
    \item \textbf{NFR-03 (Edge Resource Footprint):} The complete runtime container shall operate within $\le 2.0\text{GB}$ system RAM and $\le 4.5\text{GB}$ VRAM on target edge hardware (e.g., NVIDIA Jetson Orin Nano / RPi 5).
    \item \textbf{NFR-04 (High Aesthetic Presentation):} The administrative user interface shall render high-contrast, glassmorphic UI elements and symbolic-only skeleton overlays without displaying un-anonymized raw camera feeds.
    \item \textbf{NFR-05 (Fail-Closed Governance Semantics):} In the event of a sensor malfunction or processing exception, the governance gate shall default to a safe, access-denied Boolean state.
    \item \textbf{NFR-06 (GDPR Article 25 Compliance):} The system shall structurally satisfy Privacy-by-Design and Data Minimization requirements, rendering identity reconstruction from output metadata mathematically underdetermined.
\end{enumerate}

\begin{table}[htbp]
\caption{System Requirements Allocation & Verification Summary}
...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Req ID} & \textbf{Requirement Category} & \textbf{Target Metric / Invariant} & \textbf{Verifying Subsystem} \\
\midrule
FR-01 & Anonymous Pose Sensing & 17 Keypoints / No Raw Storage & L3 Edge Abstraction \\
FR-02 & Open-Set Identification & $\le 33\text{ms}$ Match / 100k Profiles & InsightFace + FAISS \\
FR-03 & Truancy & ST-CSF Timetable Matching & Governance Gate (L5) \\
FR-04 & Velocity Filtering & $v_i \le v_{max}$ Kinematic Bound & ST-CSF Engine \\
FR-05 & Acoustic Monitoring & Non-semantic FFT Spectral Vectors & Audio Sentinel Module \\
FR-06 & Auditability & Cryptographic Merkle Root Hash & Trust Layer (L7) \\
FR-07 & Access Control & 7-Role RBAC Enforcement & API Gateway \\
FR-08 & Federation & Federated Averaging (FedAvg) & L8 Federation Layer \\
\midrule
NFR-01 & Latency Floor & $\le 33\text{ms}$ End-to-End Pipeline & CUDA Engine \\
NFR-02 & Volatile TTL & $33\text{ms}$ Frame / $3\text{s}$ Audio Overwrite & Volatile Memory Manager \\
NFR-03 & Edge Resource Limit & $\le 2.0\text{GB}$ RAM / $\le 4.5\text{GB}$ VRAM & Jetson Hardware Baseline \\
NFR-04 & User Interface & Symbolic Skeleton Rendering & Streamlit Admin UI (L6) \\
NFR-05 & Failure Isolation & Bounded Boolean Default State & Fail-Closed Watchdog \\
NFR-06 & Privacy Compliance & GDPR Article 25 Verification & L3 Destruction Boundary \\
\bottomrule
\end{tabular}
\end{table}
...
In the ScholarMaster federated architecture (\texttt{core/canonical_layers.py}), local model gradient updates are computed independently on departmental edge nodes. The central coordinator computes a weighted average of model parameters, ensuring that raw biometric representations never leave local edge nodes while maintaining continuous global model optimization across campus.

\section{Comparative Literature Summary}
To highlight the key technological advancements of ScholarMaster over existing campus monitoring and privacy-preserving approaches, Table \ref{tab:literature_comparison} provides a systematic comparative summary across core system dimensions.

\begin{table}[htbp]
\caption{Comparative Literature Matrix Across Core Campus Sensing Paradigms}
...
\toprule
\textbf{Paradigm} & \textbf{Identity Handling} & \textbf{Privacy Guarantee} & \textbf{Latency} & \textbf{GDPR Compliance} & \textbf{Auditability} \\
\midrule
Manual / RFID & Physical ID Card & Low (Proxy risk) & Manual & Non-compliant & Vulnerable \\
CCTV / Central NVR & Raw Video Storage & None (Central leak risk) & Passive & Non-compliant & Unsealed logs \\
Differential Privacy & Perturbed Vectors & Algorithmic ($\epsilon$-noise) & Moderate & Partial & Soft logs \\
Homomorphic Enc. & Encrypted Templates & Cryptographic Proof & Very High ($>1\text{s}$) & Compliant & Heavy overhead \\
\textbf{ScholarMaster} & \textbf{Volatile RAM / Pose} & \textbf{Structural (L3 TTL)} & \textbf{Low ($\le 33\text{ms}$)} & \textbf{Full (Art. 25)} & \textbf{Merkle Tree} \\
\bottomrule
\end{tabular}
\end{table}
...
% =========================================================
\chapter{System Analysis \& Requirement Specification}
\thispagestyle{empty}
...
\section{Repository Module Traceability & Architecture Mapping}
To provide transparent traceability between the theoretical system layers (L1--L8) and the production codebase repository, Table \ref{tab:codebase_traceability} maps each architectural layer to its implementing Python module, core class structure, primary functions, and corresponding paper contract in the 21-paper ScholarMaster series.

\begin{table}[htbp]
\caption{Repository Module Traceability & Subsystem Mapping}
\label{tab:codebase_traceability}
\centering
\setstretch{1.1}
\fontsize{9}{11}\selectfont
\begin{tabular}{@{}lllll@{}}
\toprule
\textbf{Layer / Domain} & \textbf{Primary Source File} & \textbf{Core Classes / Functions} & \textbf{Paper ID} & \textbf{Operational Role} \\
\midrule
L1/L2 Physics & \texttt{audio\_sentinel.py} & \texttt{AudioSentinel}, \texttt{extract\_fft} & P5, P6 & Sensor Ingestion & Spectral FFT \\
L3 Destruction & \texttt{canonical_layers.py} & \texttt{PoseExtractor}, \texttt{VolatileManager} & P3 & 33ms TTL Overwrite \& Pose \\
L4 Local Inference & \texttt{canonical_layers.py} & \texttt{InsightFaceEngine}, \texttt{FAISSIndex} & P1, P2 & 512-D Face Match \& Gestures \\
L5 Governance & \texttt{modules_legacy/st_csf.py} & \texttt{STCSFEngine}, \texttt{check\_compliance} & P7, P9 & ST-CSF Timetable Filter \\
L6 Presentation & \texttt{admin_panel.py} & \texttt{StreamlitUI}, \texttt{render_dashboard} & P15 & Glassmorphic RBAC Dashboard \\
L7 Ephemeral RAM & \texttt{trust_layer.py} & \texttt{MerkleTreeLedger}, \texttt{SHA256Chain} & P8 & Cryptographic Audit Ledger \\
L8 Federation & \texttt{canonical_layers.py} & \texttt{FederatedAggregator}, \texttt{FedAvg} & P13, P14 & Gradient Average \& Purging \\
Edge Engine & \texttt{main.py} & \texttt{MainOrchestrator}, \texttt{ThreadLoop} & P10, P11 & 24/7 Multi-Threaded Loop \\
REST API & \texttt{api/main.py} & \texttt{FastAPIApp}, \texttt{RBACMiddleware} & P12 & Async REST API Gateway \\
\bottomrule
\end{tabular}
\end{table}
...
% =========================================================
\chapter{System Architecture \& Concentric Security Model}
\thispagestyle{empty}
...
% =========================================================
\chapter{Biometric Sensing \& Identification Layer}
\thispagestyle{empty}
...
This section presents the formal LaTeX algorithm specifications for the Biometric Sensing Layer.


\begin{algorithm}[H]
\caption{ArcFace Deep Biometric Feature Embedding Extraction}
\KwIn{Preprocessed BGR Frame $F \in \mathbb{R}^{H \times W \times 3}$, Landmark Detector $\mathcal{L}$}
\KwOut{Normalized 512-dimensional Feature Vector $\mathbf{v} \in \mathbb{R}^{512}$}
\SetKwFunction{FExtract}{ArcFaceExtract}
\SetKwFunction{FNorm}{L2Normalize}

$\mathcal{K} \leftarrow \mathcal{L}(F)$ \tcp*{Detect 5 canonical facial landmarks}
$F_{\text{crop}} \leftarrow \text{AffineAlign}(F, \mathcal{K}, 112 \times 112)$ \tcp*{Align to 112x112 standard crop}
$\mathbf{z} \leftarrow \text{ResNet100\_Backbone}(F_{\text{crop}})$ \tcp*{Forward pass through ResNet100}
$\mathbf{v} \leftarrow \frac{\mathbf{z}}{\|\mathbf{z}\|_2}$ \tcp*{Apply L2 norm: $\|\mathbf{v}\|_2 = 1.0$}
\Return{$\mathbf{v}$}\;
\end{algorithm}
...
\begin{algorithm}[H]
\caption{FAISS HNSW Open-Set Vector Indexing \& Search}
\KwIn{Query Vector $\mathbf{v} \in \mathbb{R}^{512}$, HNSW Index Graph $\mathcal{G}_{HNSW}$, Threshold $\tau(N)$}
\KwOut{Identified Student ID $S_{\text{id}}$ or UNKNOWN Flag}
\SetKwFunction{FSearch}{HNSWSearch}

$(\mathcal{I}_{\text{cand}}, \mathcal{D}_{\text{cand}}) \leftarrow \mathcal{G}_{HNSW}.\text{Search}(\mathbf{v}, K=1)$ \tcp*{Sub-5ms graph traversal}
$d_{\text{min}} \leftarrow \mathcal{D}_{\text{cand}}[0]$\;
$S_{\text{candidate}} \leftarrow \mathcal{I}_{\text{cand}}[0]$\;
\eIf{$d_{\text{min}} \le \tau(N)$}{
    $S_{\text{id}} \leftarrow S_{\text{candidate}}$ \tcp*{Match accepted within decision boundary}
}{
    $S_{\text{id}} \leftarrow \text{UNKNOWN}$ \tcp*{Open-set unknown identity rejected}
}
\Return{$S_{\text{id}}$}\;
\end{algorithm}
...
\begin{algorithm}[H]
\caption{Multi-Stage EAR \& 3D PnP Head Pose Liveness Verification}
\KwIn{Landmark Mesh $\mathcal{M}_{468}$, Camera Calibration Matrix $\mathbf{K}$}
\KwOut{Liveness Status $\text{IsLive} \in \{\text{TRUE}, \text{FALSE}\}$}

$\text{EAR} \leftarrow \frac{\|p_2-p_6\| + \|p_3-p_5\|}{2\|p_1-p_4\|}$ \tcp*{Eye Aspect Ratio computation}
$(\mathbf{R}, \mathbf{t}) \leftarrow \text{solvePnP}(\mathbf{X}_{\text{3d\_model}}, \mathbf{x}_{\text{2d\_landmarks}}, \mathbf{K})$ \tcp*{Perspective-n-Point pose solve}
$(\text{pitch}, \text{yaw}, \text{roll}) \leftarrow \text{EulerAngles}(\mathbf{R})$\;
$\theta_{\text{dev}} \leftarrow \sqrt{\text{pitch}^2 + \text{yaw}^2}$\;
$E_{\text{head}} \leftarrow \max(0.0, 1.0 - \frac{\theta_{\text{dev}}}{\theta_{\text{max}}})$\;
$E_{\text{eye}} \leftarrow \min(1.0, \frac{\text{EAR}}{0.30})$\;
$S_{\text{live}} \leftarrow 0.7 E_{\text{head}} + 0.3 E_{\text{eye}}$\;
\eIf{$S_{\text{live}} \ge 0.65 \land \text{EAR} > 0.15$}{
    $\text{IsLive} \leftarrow \text{TRUE}$\;
}{
    $\text{IsLive} \leftarrow \text{FALSE}$ \tcp*{Spoofing or severe distraction detected}
}
\Return{$\text{IsLive}$}\;
\end{algorithm}
...
\begin{algorithm}[H]
\caption{Volatile RAM Cache 33ms TTL Memory Destruction}
\KwIn{Frame Buffer Queue $\mathcal{Q}_{\text{RAM}}$, Current Time $t_{\text{now}}$, TTL Ceiling (33ms)}
\KwOut{Freed Memory Size (Bytes)}

\For{\KwSty{each} Frame $f \in \mathcal{Q}_{\text{RAM}}$}{
    $\Delta t \leftarrow t_{\text{now}} - f.\text{timestamp}$\;
    \If{$\Delta t > 33\text{ms} \lor f.\text{processed} = \text{TRUE}$}{
        $\text{memset\_zero}(f.\text{raw\_buffer})$ \tcp*{Zero out volatile memory bytes}
        $\mathcal{Q}_{\text{RAM}}.\text{remove}(f)$ \tcp*{Purge pointer reference}
    }
}
\Return{$\text{FreedBytes}$}\;
\end{algorithm}

\chapter{Spatiotemporal Compliance \& Cryptographic Audit Engine}
\thispagestyle{empty}
...
\begin{tikzpicture}[scale=0.85, every node/.style={transform shape},
    startnode/.style={circle, draw=black, fill=black, minimum size=0.4cm},
    stopnode/.style={circle, draw=black, fill=white, inner sep=0pt, minimum size=0.4cm, path picture={\draw[fill=black] (path picture bounding box.center) circle (0.12cm);}},
    action/.style={rectangle, draw=blue!70, fill=blue!5, text width=3.2cm, align=center, rounded corners=3pt, minimum height=0.8cm, font=\scriptsize},
    decision/.style={diamond, draw=orange!80, fill=orange!10, text width=2.2cm, align=center, aspect=1.8, font=\tiny},
    arrow/.style={->, >=stealth, thick, draw=blue!70!black}
]
    \node[startnode] (start) at (0, 6.0) {};
    \node[action] (ingest) at (0, 5.0) {Ingest Detection Event \\ $(S, L_c, t)$};
    \node[action] (fetch) at (0, 3.8) {Fetch Timetable \\ $(L_{\text{exp}}, t_{\text{start}}, t_{\text{end}})$};
    
    \node[decision] (velcheck) at (0, 2.2) {$V_{\text{trans}} \le v_{\max}$?};
    \node[action, right=1.5cm of velcheck] (teleport) {Flag Teleportation \\ Anomaly (Discard)};
    
    \node[decision] (loccheck) at (0, 0.4) {$L_c = L_{\text{exp}}$?};
    \node[action, left=1.2cm of loccheck] (valid) {Mark Compliant \\ Attendance};
    \node[action, right=1.2cm of loccheck] (truancy) {Trigger Truancy \\ Security Alert};
    
    \node[stopnode] (stop) at (0, -1.4) {};

    % Connectors
    \draw[arrow] (start) -- (ingest);
    \draw[arrow] (ingest) -- (fetch);
    \draw[arrow] (fetch) -- (velcheck);
    
    \draw[arrow] (velcheck) -- node[above, font=\tiny] {No} (teleport);
    \draw[arrow] (velcheck) -- node[right, font=\tiny] {Yes} (loccheck);
    
    \draw[arrow] (loccheck) -- node[above, font=\tiny] {Yes} (valid);
    \draw[arrow] (loccheck) -- node[above, font=\tiny] {No} (truancy);
    
    \draw[arrow] (valid) |- (stop);
    \draw[arrow] (truancy) |- (stop);
    \draw[arrow] (teleport) |- (stop);

\end{tikzpicture}
\caption{ST-CSF Spatiotemporal Compliance Verification Activity Diagram.}
\label{fig:stcsf_activity}
\end{figure}
...
\setstretch{1.0}
\begin{tikzpicture}[scale=0.85, every node/.style={transform shape},
    state/.style={rectangle, draw=blue!80, fill=blue!5, text width=2.8cm, align=center, rounded corners=6pt, minimum height=0.9cm, font=\scriptsize},
    destroystate/.style={rectangle, draw=red!80, fill=red!10, text width=3.2cm, align=center, rounded corners=6pt, minimum height=0.9cm, font=\scriptsize},
    arrow/.style={->, >=stealth, thick, draw=blue!70!black}
]
    \node[state] (captured) at (-4.5, 0) {\textbf{RAM Buffer Captured} \\ Volatile Frame Load};
    \node[state] (extracted) at (0, 0) {\textbf{Feature Extracted} \\ ArcFace / Pose Skeletons};
    \node[destroystate] (zeroed) at (4.5, 0) {\textbf{Explicit Zeroization} \\ TTL $\le 33\text{ms}$ Overwrite};

    \draw[arrow] (captured) -- node[above, font=\tiny] {GPU Forward Pass} (extracted);
    \draw[arrow] (extracted) -- node[above, font=\tiny] {Memory Dealloc} (zeroed);
    \draw[arrow] (zeroed) .. controls (0, -1.8) .. node[below, font=\tiny, color=red] {Zero Disk Retention Enforced} (captured);

\end{tikzpicture}
\caption{State Diagram of Volatile RAM Buffer Lifecycle and 33ms TTL Zeroization.}
\label{fig:ttl_state_diagram}
\end{figure}
...
\section{Formal Algorithmic Specifications}
This section presents the formal LaTeX algorithm specifications for the Spatiotemporal Compliance Engine.


\begin{algorithm}[H]
\caption{ST-CSF Spatiotemporal Compliance Verification Engine}
\KwIn{Student ID $S$, Camera Location $L_c$, Detection Time $t$, Timetable DB $\mathcal{T}$}
\KwOut{Compliance State $C_{\text{state}} \in \{\text{VALID}, \text{ABSENT}, \text{TELEPORT\_VIOLATION}\}$}

$(L_{\text{exp}}, t_{\text{start}}, t_{\text{end}}) \leftarrow \mathcal{T}.\text{GetExpectedClass}(S, t)$\;
$(L_{\text{prev}}, t_{\text{prev}}) \leftarrow \text{GetLastKnownLocation}(S)$\;
$v_{\text{trans}} \leftarrow \frac{\text{Haversine}(L_c, L_{\text{prev}})}{|t - t_{\text{prev}}|}$\;
\eIf{$v_{\text{trans}} > v_{\text{max}}$}{
    $C_{\text{state}} \leftarrow \text{TELEPORT\_VIOLATION}$ \tcp*{Impossible physical velocity detected}
}{
    \eIf{$L_c = L_{\text{exp}} \land t_{\text{start}} \le t \le t_{\text{end}}$}{
        $C_{\text{state}} \leftarrow \text{VALID}$ \tcp*{Automated attendance verified}
    }{
        $C_{\text{state}} \leftarrow \text{ABSENT}$ \tcp*{Unscheduled location breach}
    }
}
\Return{$C_{\text{state}}$}\;
\end{algorithm}
...
\begin{algorithm}[H]
\caption{Cryptographic Merkle Hash Chain Audit Recursion}
\KwIn{Attendance Event Log Array $\mathcal{E} = [e_1, e_2, \dots, e_m]$, Prior Hash Root $R_{t-1}$}
\KwOut{New Immutable Merkle Root $R_t$}

$\mathcal{H} \leftarrow [\text{SHA256}(e_i) \mid e_i \in \mathcal{E}]$ \tcp*{Compute leaf hashes}
\While{$|\mathcal{H}| > 1$}{
    \If{$|\mathcal{H}| \pmod 2 \neq 0$}{
        $\mathcal{H}.\text{append}(\mathcal{H}[-1])$ \tcp*{Duplicate last leaf for odd count}
    }
    $\mathcal{H}_{\text{next}} \leftarrow []$\;
    \For{$i \leftarrow 0$ \KwTo $|\mathcal{H}|-1$ \KwSty{step} 2}{
        $h_{\text{parent}} \leftarrow \text{SHA256}(\mathcal{H}[i] \mathbin{\Vert} \mathcal{H}[i+1])$\;
        $\mathcal{H}_{\text{next}}.\text{append}(h_{\text{parent}})$\;
    }
    $\mathcal{H} \leftarrow \mathcal{H}_{\text{next}}$\;
}
$R_t \leftarrow \text{SHA256}(\mathcal{H}[0] \mathbin{\Vert} R_{t-1})$ \tcp*{Chain with previous Merkle root}
\Return{$R_t$}\;
\end{algorithm}
...
\begin{algorithm}[H]
\caption{Non-Semantic Acoustic Feature Extraction \& Thresholding}
\KwIn{100ms PCM Audio Buffer $A[n]$, FFT Size $N=1024$, Noise Threshold $\tau_{\text{noise}}$}
\KwOut{Acoustic Alert Status $\text{Alert} \in \{\text{NORMAL}, \text{NOISE\_VIOLATION}\}$}

$X[k] \leftarrow \text{FFT}(A[n])$ \tcp*{Transform to frequency domain}
$C \leftarrow \frac{\sum_{k=0}^{N/2} f(k) |X[k]|}{\sum_{k=0}^{N/2} |X[k]|}$ \tcp*{Spectral Centroid}
$\text{ZCR} \leftarrow \frac{1}{2N} \sum_{n=1}^{N-1} |\text{sign}(A[n]) - \text{sign}(A[n-1])|$ \tcp*{Zero Crossing Rate}
\eIf{$C > \tau_{\text{centroid}} \land \text{ZCR} > \tau_{\text{zcr}}$}{
    $\text{Alert} \leftarrow \text{NOISE\_VIOLATION}$ \tcp*{Classroom disruption flag}
}{
    $\text{Alert} \leftarrow \text{NORMAL}$\;
}
\Return{$\text{Alert}$}\;
\end{algorithm}

\chapter{System Implementation \& Administrative Governance}
\thispagestyle{empty}
...
\setstretch{1.0}
\begin{tikzpicture}[scale=0.85, every node/.style={transform shape},
    comp/.style={rectangle, draw=blue!70, fill=blue!5, text width=3.4cm, align=center, rounded corners=3pt, minimum height=1.0cm, font=\scriptsize},
    pkg/.style={rectangle, draw=gray!60, fill=gray!5, text width=12.5cm, align=left, rounded corners=4pt, font=\small},
    arrow/.style={->, >=stealth, thick, draw=blue!70!black}
]
    % Component Container Packages
    \draw[thick, draw=blue!40, fill=blue!2, rounded corners=6pt] (-6.5, 3.2) rectangle (6.5, 5.8);
    \node[font=\bfseries\small, text=blue!80!black] at (-4.5, 5.4) {Core Sensing \& Inference Engine (\texttt{core/canonical\_layers.py})};
    
    \node[comp] (face) at (-4.2, 4.2) {\textbf{InsightFace Engine} \\ 512-D Embedding Extraction};
    \node[comp] (faiss) at (0.0, 4.2) {\textbf{FAISS Similarity Search} \\ Sub-ms HNSW Indexing};
    \node[comp] (pose) at (4.2, 4.2) {\textbf{YOLOv8-pose Engine} \\ 17 Keypoint Skeleton Sensing};

    \draw[thick, draw=orange!40, fill=orange!2, rounded corners=6pt] (-6.5, 0.0) rectangle (6.5, 2.6);
    \node[font=\bfseries\small, text=orange!80!black] at (-4.5, 2.2) {Governance \& Trust Layer (\texttt{modules\_legacy/})};
    
    \node[comp] (stcsf) at (-3.2, 1.0) {\textbf{ST-CSF Engine} \\ Timetable & Velocity Checks};
    \node[comp] (trust) at (3.2, 1.0) {\textbf{Merkle Trust Layer} \\ SHA-256 Crypto Audit Ledger};

    \draw[thick, draw=green!40, fill=green!2, rounded corners=6pt] (-6.5, -3.2) rectangle (6.5, -0.6);
    \node[font=\bfseries\small, text=green!80!black] at (-4.2, -0.9) {Presentation \& REST API Gateway};
    
    \node[comp] (ui) at (-3.2, -2.1) {\textbf{Streamlit Admin Panel} \\ 7-Role RBAC Dashboards};
    \node[comp] (api) at (3.2, -2.1) {\textbf{FastAPI Gateway} \\ Async Endpoint Handlers};

    % Connections
    \draw[arrow] (face) -- (faiss);
    \draw[arrow] (faiss) -- (stcsf);
    \draw[arrow] (pose) -- (stcsf);
    \draw[arrow] (stcsf) -- (trust);
    \draw[arrow] (trust) -- (ui);
    \draw[arrow] (trust) -- (api);

\end{tikzpicture}
\caption{Component Architecture Diagram Mapping Internal Core Packages and Imports.}
...
\setstretch{1.0}
\begin{tikzpicture}[scale=0.85, every node/.style={transform shape},
    nodebox/.style={rectangle, draw=gray!80, fill=gray!10, text width=3.4cm, align=center, rounded corners=4pt, minimum height=1.2cm, font=\scriptsize},
    edgebox/.style={rectangle, draw=blue!80, fill=blue!10, text width=4.2cm, align=center, rounded corners=4pt, minimum height=1.6cm, font=\scriptsize},
    arrow/.style={->, >=stealth, thick, draw=blue!70!black}
]
    % Physical Sensing Devices
    \node[nodebox] (cam) at (-4.8, 2.5) {\textbf{IP Camera Sensors} \\ RTSP Stream (1080p @ 30 FPS) \\ H.264 Video Stream};
    \node[nodebox] (mic) at (-4.8, -0.5) {\textbf{Classroom Microphone} \\ PortAudio Audio Stream \\ 44.1 kHz PCM Samples};

    % Edge Node Processor Container
    \draw[thick, draw=blue!60, fill=blue!2, rounded corners=6pt] (-1.5, -2.0) rectangle (3.5, 3.8);
    \node[font=\bfseries\small, text=blue!80!black] at (1.0, 3.4) {Edge Processing Node (Jetson / M2)};
    
    \node[edgebox] (cuda) at (1.0, 2.0) {\textbf{Volatile GPU Accelerator} \\ PyTorch 2.1 / CUDA / CoreML \\ Volatile RAM Buffer (TTL $\le 33\text{ms}$)};
    \node[edgebox] (daemon) at (1.0, -0.8) {\textbf{Systemd Service Daemon} \\ FastAPI REST API (Port 8000) \\ Streamlit UI (Port 8501)};

    % Client Consoles
    \node[nodebox] (ui) at (6.2, 2.5) {\textbf{HOD & Admin Terminals} \\ Streamlit Glassmorphic Panel \\ Scoped 7-Role RBAC Views};
    \node[nodebox] (ledger) at (6.2, -0.5) {\textbf{SQLite & Merkle Ledger} \\ \texttt{campus.db} & \texttt{trust\_layer.py} \\ SHA-256 Crypto Root Chains};

    % Wiring Networks
    \draw[arrow] (cam) -- node[midway, above, font=\tiny] {Gigabit LAN / RTSP} (cuda);
    \draw[arrow] (mic) -- node[midway, above, font=\tiny] {PortAudio PCM} (cuda);
    \draw[arrow] (cuda) -- node[midway, left, font=\tiny] {Internal IPC} (daemon);
    \draw[arrow] (daemon) -- node[midway, above, font=\tiny] {HTTP / WebSockets} (ui);
    \draw[arrow] (daemon) -- node[midway, above, font=\tiny] {Atomic File Lock} (ledger);

\end{tikzpicture}
\caption{Physical Hardware Deployment Topology & Network Interface Setup.}
...
\begin{tikzpicture}[scale=0.85, every node/.style={transform shape}]
    % Lifelines
    \draw[thick, fill=blue!10] (0, 7.0) rectangle (2.2, 7.6) node[midway, font=\scriptsize\bfseries] {Camera Sensor};
    \draw[thick, fill=blue!10] (3.2, 7.0) rectangle (5.4, 7.6) node[midway, font=\scriptsize\bfseries] {Video Thread};
    \draw[thick, fill=orange!10] (6.4, 7.0) rectangle (8.6, 7.6) node[midway, font=\scriptsize\bfseries] {FAISS Index};
    \draw[thick, fill=red!10] (9.6, 7.0) rectangle (11.8, 7.6) node[midway, font=\scriptsize\bfseries] {ST-CSF Filter};
    \draw[thick, fill=green!10] (12.8, 7.0) rectangle (15.0, 7.6) node[midway, font=\scriptsize\bfseries] {Merkle Ledger};

    \draw[dashed, gray] (1.1, 7.0) -- (1.1, 0.5);
    \draw[dashed, gray] (4.3, 7.0) -- (4.3, 0.5);
    \draw[dashed, gray] (7.5, 7.0) -- (7.5, 0.5);
    \draw[dashed, gray] (10.7, 7.0) -- (10.7, 0.5);
    \draw[dashed, gray] (13.9, 7.0) -- (13.9, 0.5);

    % Sequence Messages
    \draw[->, >=stealth, thick, blue!80] (1.1, 6.2) -- node[above, font=\tiny] {1. Raw Video Frame (30 FPS)} (4.3, 6.2);
    \draw[->, >=stealth, thick, blue!80] (4.3, 5.4) -- node[above, font=\tiny] {2. Extract 512-D Face Crop} (7.5, 5.4);
    \draw[->, >=stealth, thick, orange!90!black] (7.5, 4.6) -- node[above, font=\tiny] {3. Sub-ms Vector Search} (4.3, 4.6);
    \draw[->, >=stealth, thick, red!80] (4.3, 3.8) -- node[above, font=\tiny] {4. Match Timetable ($v_i \le v_{\max}$)} (10.7, 3.8);
    \draw[->, >=stealth, thick, red!80] (10.7, 3.0) -- node[above, font=\tiny] {5. Compliance Status Boolean} (4.3, 3.0);
    \draw[->, >=stealth, thick, green!70!black] (4.3, 2.2) -- node[above, font=\tiny] {6. Append SHA-256 Hash} (13.9, 2.2);
    \draw[->, >=stealth, thick, gray!80, dashed] (4.3, 1.4) -- node[above, font=\tiny, color=red] {7. Overwrite Volatile Frame (TTL $\le 33\text{ms}$)} (1.1, 1.4);

\end{tikzpicture}
\caption{Sequence Diagram of Multi-Threaded Sensing, Inference and Governance Validation.}
...
\section{Formal Algorithmic Specifications}
This section presents the formal LaTeX algorithm specification for the CSP Timetable Scheduling Engine.


\begin{algorithm}[H]
\caption{CSP Timetable Scheduling Backtracking Search with MRV/LCV}
\KwIn{Variables $\mathcal{V}$ (Courses), Domains $\mathcal{D}$ (Slots/Rooms), Constraints $\mathcal{C}$}
\KwOut{Valid Assignment $\mathcal{A}$ or FAILURE}

\SetKwFunction{FBacktrack}{Backtrack}
\SetKwProg{Fn}{Function}{:}{}
\Fn{\FBacktrack{$\mathcal{A}$}}{
    \If{$|\mathcal{A}| = |\mathcal{V}|$}{
        \Return{$\mathcal{A}$} \tcp*{Complete schedule generated}
    }
    $var \leftarrow \text{SelectUnassignedVariable\_MRV}(\mathcal{V}, \mathcal{A}, \mathcal{C})$\;
    \For{\KwSty{each} $val \in \text{OrderDomainValues\_LCV}(var, \mathcal{A}, \mathcal{C})$}{
        \If{$\text{IsConsistent}(var, val, \mathcal{A}, \mathcal{C})$}{
            $\mathcal{A}[var] \leftarrow val$\;
            $result \leftarrow \text{Backtrack}(\mathcal{A})$\;
            \If{$result \neq \text{FAILURE}$}{
                \Return{$result$}\;
            }
            $\text{remove } var \text{ from } \mathcal{A}$ \tcp*{Backtrack step}
        }
    }
    \Return{\text{FAILURE}}\;
}
\Return{\FBacktrack{$\emptyset$}}\;
\end{algorithm}

\chapter{Exploratory Data Analysis \& Experimental Setup}
\thispagestyle{empty} 
...
\section{Formal Evaluation Metrics Algorithm}
This section presents the formal LaTeX evaluation metric calculation algorithm.


\begin{algorithm}[H]
\caption{Open-Set Metric Evaluation Protocol (OSIR, UIRR, OSIE)}
\KwIn{Known Probes $\mathcal{P}_{\text{known}}$, Unknown Probes $\mathcal{P}_{\text{unknown}}$, Distance Threshold $\tau$}
\KwOut{OSIR Score, UIRR Score, OSIE Score}

$C_{\text{correct}} \leftarrow 0, U_{\text{rejected}} \leftarrow 0, U_{\text{error}} \leftarrow 0$\;
\For{\KwSty{each} $(v, y_{\text{true}}) \in \mathcal{P}_{\text{known}}$}{
    $(y_{\text{pred}}, d) \leftarrow \text{SearchIndex}(v)$\;
    \If{$d \le \tau \land y_{\text{pred}} = y_{\text{true}}$}{
        $C_{\text{correct}} \leftarrow C_{\text{correct}} + 1$\;
    }
}
\For{\KwSty{each} $(v, \text{UNKNOWN}) \in \mathcal{P}_{\text{unknown}}$}{
    $(y_{\text{pred}}, d) \leftarrow \text{SearchIndex}(v)$\;
    \eIf{$d > \tau$}{
        $U_{\text{rejected}} \leftarrow U_{\text{rejected}} + 1$\;
    }{
        $U_{\text{error}} \leftarrow U_{\text{error}} + 1$\;
    }
}
$\text{OSIR} \leftarrow \frac{C_{\text{correct}}}{|\mathcal{P}_{\text{known}}|} \times 100\%$\;
$\text{UIRR} \leftarrow \frac{U_{\text{rejected}}}{|\mathcal{P}_{\text{unknown}}|} \times 100\%$\;
$\text{OSIE} \leftarrow \frac{U_{\text{error}}}{|\mathcal{P}_{\text{unknown}}|} \times 100\%$\;
\Return{$\text{OSIR}, \text{UIRR}, \text{OSIE}$}\;
\end{algorithm}

\chapter{Empirical Evaluation, Results \& Compliance Verification}
\thispagestyle{empty}
...
\section{Cross-Paper Performance Benchmarking Summary}
To establish unified empirical evidence across the entire research program, Table \ref{tab:master_benchmarks} consolidates the verified performance metrics, latency floors, and confidence bounds across the 21-paper ScholarMaster series.

\begin{table}[htbp]
\caption{Master Cross-Paper Benchmarking & System-Wide Metric Summary}
...
\begin{tabular}{@{}lllll@{}}
\toprule
\textbf{Metric Description} & \textbf{Target Layer} & \textbf{Measured Benchmark Value} & \textbf{Constraint Bound} & \textbf{Status} \\
\midrule
Open-Set Identification Rate & L4 Local Inference & $99.2\%$ OSIR & $\ge 98.0\%$ & VERIFIED \\
Unknown Rejection Rate & L4 Local Inference & $99.5\%$ UIRR & $\ge 98.5\%$ & VERIFIED \\
ST-CSF Truancy F1-Score & L5 Governance Gate & $98.2\%$ F1-Score & $\ge 95.0\%$ & VERIFIED \\
Frame Ingestion Latency & L2 Sensor Acquisition & $14.5\text{ms}$ & $\le 33.0\text{ms}$ & VERIFIED \\
End-to-End Pipeline Latency & L1--L8 Stack & $32.4\text{ms}$ & $\le 33.0\text{ms}$ & VERIFIED \\
Volatile RAM Memory TTL & L3 Edge Abstraction & $33\text{ms}$ Frame / $3\text{s}$ Audio & $\le 33\text{ms}$ Frame & VERIFIED \\
False Alert Reduction & L5 Governance Gate & $85.0\%$ Reduction & $\ge 80.0\%$ & VERIFIED \\
Merkle Tree Audit Validation & L7 Ephemeral Memory & SHA-256 Chain Verification & $100\%$ Immutable & VERIFIED \\
\bottomrule
\end{tabular}
\end{table}
...
\setstretch{1.0}
\begin{tikzpicture}[scale=0.85, every node/.style={transform shape},
    bar/.style={rectangle, draw=blue!80, fill=blue!20, minimum height=0.6cm, font=\tiny\bfseries},
    guide/.style={dashed, gray!60}
]
    % Time Axis
    \draw[->, thick] (0, 0) -- (12.0, 0) node[right, font=\tiny] {Time (ms)};
    \foreach \x/\t in {0/0, 2/5, 4/10, 6/15, 8/20, 10/25, 11.6/33} {
        \draw (\x, 0.1) -- (\x, -0.1) node[below, font=\tiny] {\t ms};
        \draw[guide] (\x, 0) -- (\x, 4.5);
    }
    
    % Budget Line (33ms)
    \draw[ultra thick, red] (11.6, 0) -- (11.6, 4.8) node[above, font=\tiny\bfseries, color=red] {33ms Real-Time Budget Floor};

    % Execution Bars
    \node[bar, text width=1.6cm, anchor=west] at (0, 3.8) {Frame Ingest (2.1ms)};
    \node[bar, text width=2.4cm, anchor=west, fill=orange!20, draw=orange!80] at (1.8, 2.8) {ArcFace Vector Extract (8.4ms)};
    \node[bar, text width=1.2cm, anchor=west, fill=green!20, draw=green!80] at (4.4, 1.8) {FAISS Match (0.8ms)};
    \node[bar, text width=2.0cm, anchor=west, fill=purple!20, draw=purple!80] at (5.8, 0.8) {ST-CSF & Merkle Chain (3.2ms)};

\end{tikzpicture}
\caption{End-to-End Frame Processing Execution Timing Breakdown ($14.5\text{ms}$ Total vs. $33.0\text{ms}$ Budget Floor).}
