...
\definecolor{scetpurple}{RGB}{120, 30, 120} % Purple for Internal Examiner
...
\begin{titlepage}
\begin{center}
    \vspace*{6pt} % Reduced top margin cushion
    \setstretch{1.0} % Set to single spacing for layout safety
    
    % MAIN TITLE (All Caps, Black)
    {\fontsize{14}{16}\bfseries\selectfont\color{black} BEYOND THE PRIVACY-UTILITY TRADE-OFF: A SPATIOTEMPORAL-CONSTRAINT AND SECURE INTELLIGENT CAMPUS MONITORING SYSTEM\par}
    
    \vspace{4pt}
    % REPORT LEVEL (ALL CAPS - Red)
    {\fontsize{11}{13}\bfseries\selectfont\color{scetred} A PROJECT REPORT\par}
    \vspace{1pt}
    ...
    {\fontsize{12}{14}\bfseries\selectfont\color{scetred} JAWAHARLAL NEHRU TECHNOLOGICAL UNIVERSITY KAKINADA\par}
    \vspace{1pt}
    {\fontsize{10}{12}\itshape\selectfont\color{scetgreen} For the award of the degree\par}
    
    \vspace{4pt}
    % ACADEMIC DEGREE BLOCK (ALL CAPS - Red / Green "In")
    {\fontsize{13}{15}\bfseries\selectfont\color{scetred} MASTER OF TECHNOLOGY\par}
    \vspace{2pt}
    {\fontsize{10}{12}\itshape\bfseries\selectfont\color{scetgreen} In\par} 
    \vspace{2pt}
    {\fontsize{13}{15}\bfseries\selectfont\color{scetred} COMPUTER SCIENCE \& ENGINEERING\par}
    
    \vspace{4pt}
    {\fontsize{10}{12}\itshape\selectfont\color{scetgreen} Submitted\par}
    \vspace{1pt}
    {\fontsize{10}{12}\itshape\selectfont\color{scetgreen} By\par}
    \vspace{2pt}
    ...
    {\fontsize{11}{13}\bfseries\selectfont\color{scetred} Professor(CSE)/Principal\par}
...
\begin{tabular}{@{}p{0.5\textwidth}p{0.5\textwidth}@{}}
    \centering {\color{scetbrown}\textbf{Guide}} \\ [2pt] {\color{scetred}\textbf{Dr. S. Suresh Kumar, \textmd{M.Tech, Ph.D, PDF}}} \\ {\color{scetred}\small\textbf{Professor(CSE)/Principal}} \\ {\color{scetred}\small\textbf{Department of CSE}} &
...
\vspace{0.4in}
\begin{center}
    {\color{scetpurple}\textbf{Internal Examiner}}
\end{center}
...
\begin{itemize}
    ...
    \item[a.] The project work contained in the report is original and has been done by under the guidance of my supervisor.
...
\noindent\begin{tabular}{@{}p{0.35\textwidth}p{0.65\textwidth}@{}}
    ...
    \textbf{Place:} Narasapur & \textbf{Roll No:} 24A21D5827 \\ [0.15in]
...
\noindent I would like to express my sincere gratitude to Management for providing us with all the necessary facilities, constant encouragement and support throughout the course of studies.

\noindent I hugely indebted to Dr. S. Suresh Kumar, Principal for finding out time for us to share knowledge for being ever so kind to show interest in my work and for giving his precious and kind advice for the same.

\noindent I would like to express my grateful thanks to Dr. P. Srinivasulu, Professor \& Head, Department of Computer Science and Engineering, for his valuable suggestions and guidance regarding the software analysis, Design and also for his continuous effort in the successful completion of the project.

\noindent My deepest appreciation to the project coordinator Mr. N. Tulasi Raju, Associate Professor, Department of Computer Science and Engineering for his valuable guidance for efficacious completion of the project.

\noindent I also place on record, my thanks to one and all who directly or indirectly have lent their helping hand this venture.
...
\begin{itemize}
    ...
    \item \textbf{Reliability \& Thermal Controls (NFR-5):} The system must include thermal throttling controls that dynamically scale the video analysis rate from 30 FPS to 15 FPS if system temperature exceeds $85^\circ\text{C}$ to maintain stability.
...
\begin{tikzpicture}[
    ...
    db/.style={cylinder, cylinder points=20, shape border rotate=90, draw=orange!80, fill=orange!5, text width=1.8cm, align=center, minimum height=1.0cm, font=\scriptsize},
    arrow/.style={->, >=stealth, thick, draw=gray!70},
