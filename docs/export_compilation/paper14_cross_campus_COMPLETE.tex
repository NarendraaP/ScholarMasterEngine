...
No existing work addresses: (1) \textbf{Three-tier hierarchical FL} for cross-institutional collaboration, (2) \textbf{Staleness-aware campus weighting} under asynchronous connectivity, (3) \textbf{Institutional governance} with audit and rollback capabilities, (4) \textbf{Real-world academic deployment} constraints (GDPR/DPDP alignment, bandwidth limits, administrative autonomy).
...
\textbf{Defense:} Hierarchical DP (Section V) ensures $(\epsilon, \delta)$-privacy. Coordinator only sees campus-aggregated gradients with two noise layers, reducing single-student reconstruction risk.
