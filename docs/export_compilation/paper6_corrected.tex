...
% ==========================================
\begin{abstract}
Visual safety monitoring systems are inherently limited by line-of-sight occlusion, creating "Blind Spots" in complex architectural environments where safety threats often go undetected. While acoustic monitoring offers omnidirectional awareness, its adoption is constrained by strict privacy norms prohibiting continuous audio recording. This paper introduces a **Zero-Retention** acoustic sentinel designed to detect high-decibel anomalies (e.g., impulsive vocalizations, crashes $>95$dB) without retaining intelligible speech data. We employ a Volatile Memory Processing architecture that extracts spectral energy features in real-time and immediately overwrites the raw audio buffer with zeros, acting as an "architectural volatile-memory privacy barrier." We implement spectral gating heuristics adapted for safety anomaly discrimination that isolate high-energy impulse events from ambient background noise. Furthermore, we implement Generalized Cross-Correlation with Phase Transform (GCC-PHAT) for robust source localization. Experimental results demonstrate that this multi-modal approach achieves 100\% detection of staged, high-energy safety-critical impulses within the defined 5--15m Safety Envelope ($S_{env}$) under controlled experimental conditions in Non-Line-of-Sight (NLOS) scenarios, while architecturally ensuring that no reconstructible speech data remains reachable via live process-level references beyond a bounded 3-second volatile processing window (deallocated buffer pages may persist in the allocator's free list or in physical DRAM until overwritten by subsequent allocations).
\end{abstract}
...
\begin{itemize}
    ...
    \item \textbf{Blind Spot Validation:} Empirical validation demonstrating 100\% detection rates for staged, high-energy impulsive events under controlled experimental conditions within a 15m Safety Envelope ($S_{env}$) in Non-Line-of-Sight (NLOS) scenarios.
\end{itemize}
...
\end{equation}
Where $P_k$ is the normalized power spectrum. Our system is designed to ignore frames where $H(S)$ indicates intelligible speech unless the SPL exceeds the safety threshold of 95dB. Spectral entropy is used solely as a conservative suppression heuristic and is not employed as a speech recognition or classification mechanism.
...
\end{equation}
A played-back recording contains the convolution of the original room's impulse response with the current room's impulse response, resulting in an unnaturally long or distorted $RT_{60}$. This technique provides heuristic resistance to naïve replay attacks and raises the cost of non-adaptive adversaries; it does not constitute a formal anti-spoofing guarantee.
...
\begin{itemize}
    \item **Compute Node:** Raspberry Pi 4 Model B (4GB). Selected for its low cost ($<$\$50), making it viable for dense deployment in blind zones where the central Apple M2 server cannot reach \cite{b26}.
...
\caption{System Architecture Data Flow. Note the feedback loop from the decision block to the "Overwrite Memory" block , supporting privacy alignment for non-anomalous audio.}
\label{fig:arch}
\end{figure}
...
Critically, we use the `mlock()` system call (in C++) or `numpy.zeros` (in Python) to prevent swapping to persistent storage and ensure that "deleted" data is physically overwritten in volatile memory. System is configured with disabled swap and secure boot; crash-dump persistence is disabled.
...
We utilized a hybrid dataset comprising public archives and synthesized campus noise. Table II breaks down the classes used. The evaluation focuses on high-energy impulsive safety events and does not represent open-world acoustic classification. No conversational speech was retained or analyzed. Samples drawn from FSD50K were used exclusively for offline validation of spectral suppression behavior and were never processed or stored within the volatile-memory pipeline.
...
\bottomrule
\multicolumn{3}{l}{\textsuperscript{*}\footnotesize Used solely to validate suppression behavior below SPL threshold.}
\end{tabular}
\end{center}
\label{tab:dataset}
\end{table}
...
\subsection{Privacy and Data Minimization Guarantees}
The system enforces data minimization by construction. Raw acoustic samples are processed exclusively in volatile memory and are overwritten within bounded time windows. As a result, the system does not retain data via live process-level references that could enable retrospective content reconstruction, speaker identification, or semantic analysis. Privacy is therefore treated as an architectural constraint enforced at the memory-management level, though—as with all managed-runtime environments—deallocated buffer pages may persist in the allocator's free list or in physical DRAM until overwritten by subsequent allocations; this is not a claim of hardware-level memory zeroing. While features are non-invertible by design, we do not provide formal information-theoretic proofs or cryptographic guarantees of non-invertibility. The privacy architecture relies on:
\begin{enumerate}
    ...
    \item \textbf{Metadata Minimization:} Event metadata transmitted to the central system via MQTT contains timestamp, device ID, and coarse angular localization only; no waveform or spectral data is ever transmitted.
\end{enumerate}

\textbf{Important Disclaimer:} These technical guarantees constitute architectural data minimization, acting as an architectural volatile-memory privacy barrier, but do not constitute formal legal compliance certification. Institutional deployment requires independent legal review of applicable data protection regulations (e.g., GDPR Article 25 'Privacy by Design'). Side-channel attacks (e.g., power or EM analysis) are outside the scope of this work.

\subsection{Deployment Constraints}
The current GCC-PHAT implementation assumes a 2D plane. In stairwells, elevation estimation is required. Extending the system to a tetrahedral microphone configuration  would allow for 3D localization (Azimuth and Elevation), crucial for multi-story tracking. Furthermore, extremely high-noise environments might require noise-robust optimization techniques similar to Adam or pre-processing steps which were excluded here for latency reasons.

% ==========================================
% VIII. CONCLUSION
% ==========================================
\section{Conclusion}
This paper presented a **Zero-Retention** acoustic monitoring system designed to eliminate blind spots in campus safety systems. By leveraging Volatile Memory Processing and GCC-PHAT localization, we achieved 100\% detection of staged, high-energy safety-critical impulses within the defined 5–15m Safety Envelope ($S_{env}$) under controlled experimental conditions in Non-Line-of-Sight (NLOS) scenarios, without recording intelligible speech. The system successfully differentiates between threats and background noise using physics-based heuristics, ensuring robust performance even when visual sensors are occluded. This architecture provides a blueprint for ethical, multi-modal security systems. The reported accuracy reflects anomaly detection performance and is not intended as a measure of semantic sound classification. This result reflects controlled staging of impulsive events and does not imply perfect detection under all real-world acoustic conditions. Future work will evaluate robustness under uncontrolled ambient noise.
...
% --- CORE ACOUSTIC SURVEILLANCE & PRIVACY ---
...
\bibitem{b24} A. R. Beresford and F. Stajano, "Location privacy in pervasive computing," \textit{IEEE Pervasive Computing}, vol. 2, no. 1, pp. 46-55, 2003.
...
% --- PRIVACY COMPARISON (VIDEO) ---
\bibitem{b16} E. B. Nievas, O. D. Suarez, G. B. Garcia, and R. Sukthankar, "Violence Detection in Video using Computer Vision Techniques," in \textit{Computer Analysis of Images and Patterns (CAIP)}, 2011.
\bibitem{b17} F. Ullah, A. Mohammad, and A. Khatoon, "A Review on State-of-the-Art Violence Detection Techniques," \textit{IEEE Access}, vol. 12, 2024.
