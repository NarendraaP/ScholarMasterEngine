...
% ==========================================
\begin{abstract}
In the era of GDPR and strict data protection laws, the integrity of academic monitoring data is as critical as its accuracy. Traditional SQL databases allow administrators to retroactively alter attendance records, creating a ``Trust Deficit'' between students and the administration. This paper introduces a \textbf{Cryptographic Provenance Layer (CPL)} designed to enforce immutability and auditability in smart campus ecosystems. We implement a **Batched Merkle-Tree Block Architecture** where attendance events are cryptographically hashed into blocks and linked via SHA-256 pointers, creating a tamper-evident chain of custody optimized for the high-frequency burst auditability patterns typical of academic schedules. Furthermore, we outline a conceptual extensibility path toward **Zero-Knowledge Proof–based audits** for high-sensitivity jurisdictions and propose a **Per-Identity Symmetric Key (PISK)** management protocol that enables ``Cryptographic Shredding,'' allowing functional data erasure via cryptographic key destruction, a mechanism widely discussed as GDPR-compatible, subject to regulatory interpretation, while preserving the structural integrity of the immutable ledger. Architectural analysis and Caliper-based benchmarking on a private Hyperledger Fabric test configuration demonstrate that the system supports burst audit workloads on the order of 1,000 TPS under Caliper-based test configurations during class transitions with negligible storage overhead, ensuring academic data remains cryptographically tamper-evident and verifiable. All throughput figures are derived from Hyperledger Caliper simulations on a private Fabric test network and do not represent production campus guarantees. This paper presents the **Trust and Audit Subsystem within the Smart Campus architecture**.
\end{abstract}
...
\IEEEPARstart{A}{cademic} analytics systems are increasingly used for high-stakes decisions, such as grading based on attendance, scholarship disbursement, or barring students from exams due to truancy. However, the centralized databases (SQL/NoSQL) powering these systems have a fundamental flaw: they are mutable. In traditional SQL-based systems, a system administrator with root access can alter historical records—marking an absent student as present, or vice versa—without cryptographic audit trails to detect tampering. This work is rigorously aligned with the Unified Reference Model \cite{P20} and adheres to the Formal Foundations of Distributed Trust \cite{P21} established for the ScholarMaster architecture.

This vulnerability creates a ``Trust Deficit.'' If a student disputes an attendance record, it becomes their word against the database, with no way to mathematically prove the integrity of the log. Furthermore, the introduction of the General Data Protection Regulation (GDPR) mandates the ``Right to be Forgotten,'' which seemingly conflicts with the concept of immutable logs \cite{b23, b9}.

We propose a solution that bridges this gap using Distributed Ledger Technology (DLT) tailored for IoT-enabled smart campuses \cite{b20, b21}. By treating every attendance event as a transaction in a cryptographically linked chain, we create a system where history cannot be rewritten without detection, assuming the Trusted Computing Base (TCB) — including root credentials and ordering-service keys — remains uncompromised; under root compromise, all integrity invariants collapse. Crucially, we introduce a **``Cryptographic Shredding''** mechanism based on Per-Identity Symmetric Keys (PISK) to allow for GDPR compliance within an immutable structure.

\subsection{Relation to the ScholarMaster Research Series}
This work focuses exclusively on the cryptographic data provenance and immutable storage layer of the ScholarMaster ecosystem. Unlike prior works in this series that address biometric sensing \cite{b19}, spatiotemporal compliance logic, or hardware efficiency, this study investigates the post-sensing data lifecycle. It assumes the existence of valid sensor inputs and solves the orthogonal problem of trust, auditability, and regulatory compliance (GDPR) in adversarial administrative environments. Unlike the spatiotemporal compliance module, this paper assumes logical validity and focuses exclusively on post-hoc trust, auditability, and regulatory defensibility of recorded events.

\subsection{Reproducibility and Integration}
The Cryptographic Provenance Layer (CPL) is implemented as part of the \textbf{ScholarMaster Engine}. Reference prototype implementations demonstrating Merkle-Tree construction, audit proof generation, and key management prototypes (including \texttt{KeyManagementService} and \texttt{ZeroKnowledgeProver} classes) are available in the open-source repository \cite{scholarmaster_repo}. Full-scale Hyperledger Fabric network deployment and consensus validation at scale remain future work.

\subsection{Key Contributions}
Our specific contributions are:
\begin{enumerate}
    \item **Batched Merkle-Tree Architecture:** A high-throughput block structure that hashes thousands of concurrent attendance events into a single Merkle Root for efficient on-chain storage, specifically integrated to handle academic attendance burst patterns.
    ...
    \item **Burst Auditability:** A system design optimized for the "morning bell" load pattern, handling high-frequency, short-interval audit bursts typical of academic schedules.
\end{enumerate}
...
To efficiently verify a single student's attendance without traversing the entire chain, we organize daily records into a Merkle Tree \cite{b6}.
Let $L_1, L_2, \dots, L_k$ be the hashed attendance records for a class. These form the leaves of the tree.
...
To enable privacy-preserving attendance verification, we outline a conceptual extensibility path demonstrating how Non-Interactive Zero-Knowledge Proofs (NIZK) could be integrated. The Fiat-Shamir protocol shown represents the cryptographic primitive; practical deployment relies primarily on the Merkle proof verification (Algorithm 1), reserving ZKP as an option for high-sensitivity jurisdictions. Similar privacy-preserving smart contract models have been explored in Hawk \cite{b13} and Zerocash \cite{b12}.
...
\textbf{Implementation \& Deployment Scope:} This paper presents the architectural design and cryptographic protocols for a blockchain-based attendance provenance system. Core components—Merkle-DAG construction (Section III.2), proof verification (Algorithm 1), and cryptographic shredding (Algorithm 2)—are implemented as reference prototypes demonstrating feasibility. The Hyperledger Fabric architecture (Section IV.1-IV.4) represents the proposed enterprise deployment model. Performance benchmarks (Figure 3-4, Table II-III) are derived from Caliper simulations on a test network rather than production campus deployment. Full-scale consensus validation, multi-organizational MSP configuration, and legal GDPR compliance validation remain future work.
}}
\end{center}

\subsection{Irreversibility Binding}
The CPL strictly operates on post-L3 irreversible outputs defined in the ScholarMaster runtime architecture. No raw biometric frames, embeddings, or reversible identity templates are ever written to the ledger or encrypted off-chain storage. The ledger stores only logical attendance assertions (e.g., "Student X present at Time T") and their cryptographic fingerprints. This ensures that a compromise of the ledger does not result in a biometric data leak.

\subsection{Hyperledger Fabric Architecture}
We propose **Hyperledger Fabric** as the reference blockchain framework due to its permissioned network model and high throughput capabilities suitable for enterprise deployment. The architecture presented represents the target deployment design. The architecture separates the transaction execution from ordering, allowing parallel processing \cite{b14}.



* **Peers:** Each Department (CS, EE, Admin) runs a Peer node. Peers maintain the ledger and execute chaincode containers.
* **Orderer:** A centralized Raft-based ordering service orders transactions into blocks. This prevents permanent forks under normal Raft quorum assumptions.
...
\begin{itemize}
    \item **Root CA:** The University IT Department acts as the Root Certificate Authority. The Root CA private key should be hardware-backed (HSM) to prevent institutional key compromise.
...
To allow students to audit their own records, we implement the Merkle Proof Verification algorithm on the client side (Algorithm 1). This ensures that verification load is offloaded to the client, preserving server resources. Node ordering during hash concatenation is determined lexicographically to ensure deterministic Merkle path reconstruction.
...
Algorithm 2 details the procedure for handling a "Right to be Forgotten" request. This effectively removes the plaintext data while leaving the immutable hash chain intact, ensuring the ledger's structural integrity is not compromised by data deletion requests. Crucially, the student identifier is hashed before logging to prevent the deletion record itself from becoming a persistent identity artifact.

\begin{algorithm}[H]
\caption{GDPR Deletion Request}
\begin{algorithmic}[1]
\REQUIRE StudentID $S$, AdminKey $K_{admin}$
...
\STATE $\text{Ledger.Append}(\text{"DELETED: "} + S_{hash})$
...
\end{tikzpicture}
\caption{Sequence Diagram of the Zero-Knowledge Audit Protocol. The student proves possession of a valid attendance token without transmitting the token itself.}
\label{fig:zkp_seq}
\end{figure}
...
We benchmarked the system using **Hyperledger Caliper** \cite{b22}. We varied the transaction load from 100 to 2,000 transactions per second (TPS). To contextualize this capacity, we define a theoretical worst-case burst model for a large campus of $N=50,000$ students (representative of major public university enrollments, which typically range from 30,000 to 60,000). Assuming an 80\% simultaneous class transition rate ($N_{active} = 40,000$) compressed into a strict 60-second "bell burst" window, the required peak throughput is $TPS_{req} = 40,000 / 60 \approx 667$ TPS. Because our architecture maintains stable latency ($<500$ms) up to 1,000 TPS, it provides a 33\% safety margin over the theoretical peak academic load. In our configuration, we observed that setting the block size $>50$ caused latency spikes due to Raft leader election timeouts during high load, leading us to optimize for smaller, more frequent blocks (BatchTimeout = 2s, BatchSize = 10).
...
Table III compares the proposed Blockchain solution with a standard MySQL database. While standard SQL databases can maintain audit logs, they generally lack the native cryptographic enforcement required to prevent retroactive tampering by administrators with root access. The blockchain introduces higher write latency but offers tamper-evident features critical for non-repudiation.
...
Tamper Resistance & Dependent on Admin Access & Crypto-Enforced (Quorum-bound) \\
...
\midrule
\textbf{Data Remanence} & Recovering deleted student data & \textbf{Key Deletion} (Crypto-shredding) \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
A potential threat is a collusion between the System Admin and the Department Head to forge attendance. In our model, consensus requires $>50\%$ of peers. For a forgery to succeed, the attacker must compromise the keys of the Exam Cell, Student Affairs, and the IT Department simultaneously. Requires majority compromise.
While PISK reduces blast radius, compromise of the institutional KMS remains a residual risk. Full legal enforcement and institutional KMS hardening (e.g., via hardware-backed HSMs) remain deployment-specific future work. This risk is equivalent to that of any enterprise encryption system and is orthogonal to ledger immutability.
...
The primary trade-off is storage growth. An immutable ledger only grows; it never shrinks. However, our benchmarks show that 10,000 daily records consume only 1.8 MB (measured as raw Fabric block size, including endorsement signatures and channel metadata). An entire academic year would consume approximately 650 MB, which is negligible for modern storage systems. To mitigate long-term growth, we implement a "state pruning" policy where data older than 4 years (graduation cycle) is archived to cold storage. This process archives and prunes the active world state database, while preserving the underlying historical blocks and their Merkle Roots on the immutable ledger.

\subsection{Legal Defensibility (GDPR)}
Under current GDPR interpretations (Recital 26), irreversible cryptographic key destruction is widely accepted as a functional equivalent of data erasure, as the personal data becomes permanently inaccessible. Our system's ``Cryptographic Shredding'' mechanism thus allows functional compliance with GDPR erasure requirements via cryptographic key destruction, widely interpreted as a functional equivalent of data erasure under GDPR Recital 26, subject to regulatory interpretation. However, we note that the efficacy of cryptographic shredding relies on strict operational governance of the KMS, introducing a human factor risk that must be managed through institutional policy.

% ==========================================
% IX. CONCLUSION
% ==========================================
\section{Conclusion}
This paper presents the ``Trust Layer'' of the Smart Campus architecture. By migrating from a trust-based model (trusting the database admin) to a proof-based model (cryptographic verification), we ensure that academic data is immutable and auditable. Our results demonstrate that permissioned blockchains like Hyperledger Fabric can handle the high-throughput academic burst load typical of large campuses while offering features that traditional SQL databases cannot natively guarantee: tamper evidence and self-sovereign verification. This work deliberately excludes event validity determination, which is addressed separately in the ScholarMaster spatiotemporal compliance module. Future work will explore the integration of ``Smart Diplomas'' that auto-generate upon the completion of attendance requirements. The PISK-based cryptographic shredding mechanism is implemented as a reference protocol demonstrating functional GDPR compliance via key destruction; full legal enforcement and institutional KMS hardening remain deployment-specific future work.

\section*{Salami-Slicing and Scope Limitation Declaration}
This manuscript addresses \textbf{cryptographic metadata provenance and immutable storage architecture} within the ScholarMaster Research Series.

\textbf{What This Paper DOES:}
\begin{itemize}
    \item Batched Merkle-Tree block architecture for high-throughput academic burst loads.
    \item Formalizes Per-Identity Symmetric Key (PISK) management for GDPR-compliant cryptographic shredding.
    \item Evaluates consensus performance and ledger throughput using Hyperledger Caliper.
    \item Outlines extensibility paths for Zero-Knowledge Proof (ZKP) audits.
\end{itemize}

\textbf{What This Paper EXPLICITLY DOES NOT DO:}
\begin{itemize}
    \item ❌ Biometric identification or face recognition (Paper 1).
    \item ❌ Spatiotemporal constraint evaluation or compliance logic (Paper 4).
    \item ❌ Formal logical consistency proofs (Paper 7).
    \item ❌ Runtime enforcement of volatile memory boundaries (Paper 18).
\end{itemize}

This work evaluates the trust, auditability, and regulatory defensibility of post-inference metadata, operating entirely orthogonally to the sensing and reasoning layers.
