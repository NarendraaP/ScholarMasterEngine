...
% ==========================================
\begin{abstract}
Automated campus monitoring systems generate massive volumes of event data---up to 1,200 events per minute in high-density scenarios---creating a ``Cognitive Bottle-neck'' for human operators. Traditional 2D dashboard interfaces force security personnel to mentally map tabular logs (e.g., ``Alert: Corridor B, Zone 4'') to physical space, increasing reaction time and inducing alert fatigue. This paper presents an \textbf{Augmented Reality (AR) Interface} for the ScholarMaster Engine that translates abstract sensor metadata into spatially-anchored visual cues. We introduce a \textbf{Location-Based Anchoring Protocol} that subscribes to the engine's MQTT telemetry and renders safety anomalies (e.g., acoustic triggers, crowding events) as 3D holographic overlays on the operator's mobile viewport. Comparative user studies ($N=20$) demonstrate that the AR interface reduces \textbf{Time-to-Action (TTA)} by 42\% compared to standard list-based dashboards under the tested campus navigation scenario, and lowers the NASA-TLX cognitive load score during multi-event stress tests. This work validates that spatial visualization is essential for closing the ``Last Mile'' gap between edge-AI detection and human intervention.
\end{abstract}
...
Any event-driven campus telemetry system (such as the ScholarMaster Engine evaluated in our prior work) provides a robust backend for detecting safety anomalies, engagement states, and schedule violations. However, the sheer velocity of data produced by the engine---potentially hundreds of concurrent events across a campus---presents a Human-Computer Interaction (HCI) challenge.  This work is rigorously aligned with the Unified Reference Model \cite{P20} and adheres to the Formal Foundations of Distributed Trust \cite{P21} established for the ScholarMaster architecture.
...
This paper focuses on the \textbf{Presentation Layer} of the ecosystem. We do not propose new detection algorithms but evaluate a novel visualization modality for existing edge-AI outputs. Unlike the visible privacy mechanisms discussed elsewhere in the series, this work evaluates AR strictly as a cognitive performance interface and does not address governance or architectural irreversibility constraints.

\subsection{Relation to the ScholarMaster Research Series}
This paper addresses the \textbf{Human Interface Layer} of the ScholarMaster system.
\begin{itemize}
    \item It consumes the \textbf{Acoustic Events} generated by Paper 6.
    \item It utilizes the \textbf{MQTT Architecture} defined in Paper 11.
    \item It adheres to the \textbf{Privacy constraints} of Paper 3 (no face rendering, only symbolic icons).
\end{itemize}
This work bridges the gap between the computational backend and the human operator.
...
Endsley defines Situation Awareness (SA) in three levels:
\begin{enumerate}
...
The AR Interface is a lightweight client operating on standard mobile hardware (Android/iOS), consuming data from the Edge Nodes.
...
\begin{tikzpicture}[node distance=2cm, auto, thick]
    % Nodes
    \node (edge) [draw, rectangle, fill=blue!10, text width=2.5cm, align=center] {Edge Node\\(Paper 6/9)};
...
To maintain the privacy constraints of Paper 3 and Paper 8, the AR interface does not display camera frames from monitored spaces; it only displays the operator's own AR view.
\begin{itemize}
...
\end{itemize}
This ensures that even if an unauthorized user views the AR screen, they see only metadata, not identities.
...
Mobile AR frameworks (ARCore/ARKit) suffer from ``Drift''---the gradual accumulation of position error ($\epsilon_{pos}$) as the user moves away from the anchor. To mitigate this, we implement an Extended Kalman Filter (EKF) that fuses the IMU data with periodic visual relocalization.
...
\begin{itemize}
    ...
    \item \textbf{Task:} ``Navigate to the location of the anomaly as fast as possible.''
...
We simulated a ``Multi-Point Failure'' scenario to induce high cognitive load:
...
We analyzed the results through the lens of Fitts's Law, which predicts the time $T$ required to move to a target:
\begin{equation}
...
Participants completed the NASA Task Load Index (TLX) survey post-experiment. We report the three most discriminative TLX dimensions; full TLX scores are included in supplementary material.
...
We measured TTA over 5 consecutive trials to assess learnability.
\begin{itemize}
...
To assess the viability of prolonged usage on the client side, we profiled the power consumption of the AR application on a reference device (Pixel 6).
...
The primary advantage of the AR architecture is enabling ``Heads-Up'' monitoring. Security staff can maintain eye contact with the environment while receiving data. This creates superior \textbf{Situational Awareness} compared to ``Heads-Down'' dashboard usage, where the operator is blind to their surroundings while reading logs.
...
The current implementation relies on mobile phones. Extended use drains battery ($\approx$ 15\% per hour). Future iterations could leverage lightweight AR glasses (e.g., Vuzix or HoloLens) for hands-free operation, though costs remain a barrier for academic deployment. While the study involved a limited number of participants ($N=20$), the consistency of effects across participant roles suggests robustness, though larger multi-site studies are planned. The observed reduction in cognitive load should not be conflated with trust formation mechanisms described elsewhere in the series.

% ==========================================
% X. CONCLUSION
% ==========================================
\section{Conclusion}
This work closes the loop between the Edge Node's detection capabilities and the human operator's response capabilities. By visualizing abstract data through Spatially-Anchored AR, we reduced the cognitive load of campus monitoring and improved reaction times by 42\% under the tested scenarios. Our findings suggest that for high-throughput AI systems, the limiting factor is often not the algorithm, but the human interface. The proposed AR architecture ensures that the high-volume inferences generated by the engine result in actionable, low-stress intelligence rather than data fatigue. The ScholarMasterEngine repository provides reference implementation code only and contains no datasets, video streams, or personally identifiable information.

\section*{Declaration of Independent Scope}
This paper addresses the \textbf{Human-Computer Interaction (HCI)} layer of the ScholarMaster series. It does not introduce new sensing models (Papers 1, 3, 6) or control logic (Paper 9), but evaluates the \textit{presentation} of those signals to human operators.
...
% --- SERIES LINKS ---
\bibitem{p2} Narendra Babu P, ``A Context-Aware Multi-Modal Framework,'' \textit{ScholarMaster Series}, 2025.
\bibitem{p3} Narendra Babu P, ``Privacy-Preserving Academic Engagement Metrics,'' \textit{ScholarMaster Series}, 2025.
\bibitem{p4} Narendra Babu P, ``Automated Schedule-Compliance Monitoring,'' \textit{ScholarMaster Series}, 2025.
\bibitem{p6} Narendra Babu P, ``Privacy-Preserving Acoustic Anomaly Detection,'' \textit{ScholarMaster Series}, 2025.
