...
Source localization depends on the physics of acoustic waves propagation \cite{b9}. The microphone arrays approach has shown its efficiency for detecting screams and gun shots \cite{b5}; however, indoor environment with corridors causes problems of reverberations \cite{b25}.
...
Propagations of sound waves in long hallways are different compared to the ideal situation where sound propagation occurs in a free space condition. The inverse square law of sound attenuation can only be applied when there is pure propagation of sound energy without any effect of reflection.
...
\bibitem{b34} P. Narendra et al., "Memory-Bound Edge Efficiency Envelope (MBEEE): A Hardware-Level Analytical Model," \textit{Journal for Basic Sciences}, vol. 26, no. 5, 2026.
