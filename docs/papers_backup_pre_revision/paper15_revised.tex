...
Let a mobile AR viewport process $M$ active spatially-anchored alert beacons with EKF pose state updates $x_k \in \mathbb{R}^6$ at 60 Hz. The end-to-end projection latency $T_{\mathrm{render}}$ through quaternion transform $P_{\mathrm{render}} = P_{\mathrm{cam}} + Q_{\mathrm{rel}}(P_{\mathrm{tgt}} - P_{\mathrm{anc}})Q_{\mathrm{rel}}^{-1}$ satisfies:
\begin{equation}
