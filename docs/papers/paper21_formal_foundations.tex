...
Let $\mathcal{A}$ be an adversary capable of injecting forged observation vectors $O'_{t}$ into the data stream prior to L5 Governance validation. The aim of $\mathcal{A}$ is to ensure that $\Phi_{comp}(\epsilon, t) = 1$ (appear compliant) while the true state $W_t \notin \Auth(\epsilon, t)$ (remains non-compliant). Let the immediate reward function $R(q, a)$ be:
\begin{equation}
