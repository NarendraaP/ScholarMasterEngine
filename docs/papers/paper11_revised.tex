...
The dominant thesis of this paper is that \textbf{immutable appliance design substantially improves operational recoverability under unstable power and connectivity conditions}. Once installed in unattended environments, reliability becomes dominated by operating-system resilience, update safety, and autonomous recovery mechanisms rather than raw application performance \cite{b1, b2, b3}.

\subsection{Operational Fragility of Mutable Edge Hosts}
Conventional Linux deployments relying on writable root filesystems accumulate configuration drift through mutable package-level updates, uncontrolled writes, and incremental state mutations. Over time, this drift amplifies corruption exposure: each power interruption risks filesystem inconsistency, and each uncoordinated update risks dependency breakage \cite{b4, b12, b17}. Unattended devices cannot rely on manual remediation---intermittent infrastructure conditions (unstable power, wireless congestion, restricted physical access) destabilize traditional deployment assumptions. Lifecycle instability emerges structurally from mutability, not merely from poor maintenance practices.
...
This invariant expresses the design principle that the persistent host OS state remains unchanged during routine operation. Invariance is enforced through \textbf{mutation confinement}: runtime mutations are restricted either to volatile memory ($V$)---which is destroyed on power loss---or to versioned container layers ($C$) managed through controlled OTA transitions. The distinction is structurally important: persistent immutable state ($H_0$) is never written during normal operation; volatile operational state ($V$) absorbs all runtime churn; containerized application state ($C$) changes only through validated OTA transitions; and transient runtime mutation exists exclusively in RAM. This confinement ensures that corruption domains are spatially bounded---repeated power interruptions cannot accumulate filesystem inconsistencies in the persistent layer.
...
The TPM-backed certificate lifecycle limits key extraction risk by generating private keys within protected hardware. Secure Boot enforcement further reduces the likelihood that unsigned kernels or modified bootloaders can be executed on the device. These security mechanisms exist primarily to protect lifecycle integrity---provisioning trust anchors stabilize deployment authenticity, and TPM-backed identity reduces unauthorized update risk. Detailed adversarial formalization and threat-model analysis are addressed in the formal threat-modeling layer of the broader series.
