...
To the best of our knowledge, no prior work formalizes the complete lifecycle management of privacy-preserving academic stewardship systems under these specific constraints. This paper focuses on the \textbf{Systems Engineering} required to operationalize the ScholarMaster algorithms, presenting an immutable, containerized architecture designed for "Zero-Touch" administration.
...
Given that the devices operate on an open campus network, standard firewalling is insufficient. We implement a defense-in-depth security architecture centered on mutual authentication.
...
To prevent physical tampering (e.g., loading a malicious kernel), we enable \textbf{Secure Boot} on the Jetson platform. The bootloader (U-Boot) signature is fused into the SoC eFuses. If the kernel signature does not match the fused key, the boot process halts, rendering the hardware unusable for unauthorized workloads (assuming eFuse integrity; boot chain integrity is assumed, not independently verified by the application layer).
