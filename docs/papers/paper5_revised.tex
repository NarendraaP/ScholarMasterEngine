...
For compressed tabular or textual workloads, the interconnect is rarely a bottleneck. Vision workloads differ. High-resolution frames constitute dense, uncompressed multidimensional arrays processed at 30–60 FPS. The analytical model captures this phenomenon: the PCIe bus becomes a serialized chokepoint, mathematically capping throughput, giving rise to the classical “Memory Wall.” Arithmetic logic units idle while awaiting tensor payloads. The theoretical compute capacity is bounded by the data delivery rate. Hardware data-paths determine the rate at which workloads access computational resources.
...
Thermal dynamics become a dominant constraint when inference nodes execute continuously rather than in short bursts. Thermal behavior acts as a primary constraint in edge modeling. Unlike data centers, many edge nodes operate without active cooling or with minimal airflow. Under continuous inference, steady-state junction temperature becomes a governing operational limit rather than a transient condition.
...
FP16 emerges as a practical Pareto point within the UMA model. It halves bandwidth pressure and more than doubles throughput with negligible accuracy degradation. INT8 offers further gains but introduces nonlinear accuracy loss in some models \cite{b37, b38}. Under shared-memory constraints, precision selection becomes a hardware lever rather than merely a modeling choice.

\subsection{Limits of the Analytical Model}
The MBEEE defines structural bounds, not exact real-world predictions. Practical operation introduces variability: scheduler jitter, thermal interface imperfections, and power delivery fluctuations. Nevertheless, certain constraints remain physics-bound: $pJ/bit$ transfer energy, queueing variance behavior, and exponential temperature acceleration. These are not software artifacts but hardware realities.
...
Within the modeled hardware constraints, UMA nodes demonstrate higher performance-per-watt and a broader feasible operating envelope. The physical modeling suggests that UMA can sustain stable node-level performance under strict latency constraints. Theoretical responsiveness remains consistent across varying analytical workloads.
