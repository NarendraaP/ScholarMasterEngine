...
For standard consumer-grade SD cards, the over-provisioning is typically low (often under $\alpha \approx 0.05$ \cite{b25}). Under such conditions, and for sustained random writes, the theoretical WAF is significantly inflated. In practice, WAF is the product of filesystem-level, FTL-level, and garbage-collection amplification, which all add up multiplicatively. For embedded systems, this means accelerated wear of the storage media, unless the host OS can shape the write-stream before it is committed to flash \cite{b9}.
...
To squeeze maximum endurance from the filesystem, specific mount configurations are required. Setting \texttt{noatime} ensures that simple read operations do not trigger a cascade of metadata timestamp updates. Enabling \texttt{background\_gc=on} permits the filesystem to aggressively reclaim blocks during low-utilization idle cycles. Additionally, it is critical practice to explicitly disable the \texttt{discard} mount option on low-tier consumer SD cards; synchronous TRIM commands often overwhelm and stall inexpensive FTL microcontrollers.
