...
The dominant thesis of this paper is that \textbf{host-side I/O shaping substantially reduces write amplification and extends flash endurance in embedded edge systems}. Edge computing deployments frequently rely on inexpensive flash storage to maintain affordability and simplify hardware replacement. Among these options, Secure Digital (SD) cards remain widely used for their cost efficiency and hardware compatibility.

\subsection{Why Continuous Edge Telemetry Accelerates NAND Wear}
Consumer SD cards are optimized for burst-oriented usage patterns---intermittent photo capture, sequential video recording, and occasional firmware updates. Embedded AI workloads violate these assumptions fundamentally. Continuous telemetry logging, container metadata churn, system swap activity, and routine OS updates generate persistent small-write patterns that aggressively stress low over-provisioned flash controllers \cite{b26}. Metadata-heavy telemetry amplifies FTL garbage collection: each 4KB JSON append can trigger internal read-modify-write cycles across multi-megabyte erase blocks. Low over-provisioning ($\alpha \approx 0.05$) magnifies this physical wear because the controller maintains fewer spare blocks for garbage-collection victim selection. The result is that workload structure---not storage capacity---drives endurance collapse \cite{b10}.
...
For standard consumer SD cards, over-provisioning is typically low (often $\alpha \approx 0.05$ \cite{b25}). Under sustained random-write conditions with such limited spare area, theoretical WAF increases significantly. Critically, WAF emerges from cross-layer interaction---filesystem-level amplification (journal rewrites, metadata cascades), FTL-level amplification (victim-block valid-page copying), and garbage-collection amplification (RMW cycles during block reclamation) compound multiplicatively. In an embedded deployment, this mathematical reality accelerates wear unless the host OS actively reshapes the write stream before it reaches the hardware \cite{b9}.
...
We acknowledge the tradeoff: up to 1 hour of telemetry data resides exclusively in the volatile dirty page cache and will be lost during a sudden power outage. This is a deliberate \textbf{durability--endurance balancing mechanism}---the system intentionally sacrifices short-term persistence for long-term hardware survival. In embedded deployments where devices are physically difficult to reach, accepting reduced durability for transient telemetry is preferable to accelerating permanent media failure. This tradeoff should appear consistently in the interpretation of all subsequent results: endurance optimization inherently introduces crash-risk exposure.
...
F2FS addresses this issue by introducing the \textbf{Node Address Table (NAT)} alongside the Segment Information Table (SIT). The NAT acts as a static indirection layer: when a data block physically moves, only the direct node block and the specific NAT entry require updating. By isolating the update cascade, the NAT largely mitigates the Wandering Tree problem. The result is a substantial reduction in metadata-related amplification \cite{b22}. F2FS is valuable here not because it universally outperforms Ext4 in all environments, but because it aligns host write geometry with NAND physics---sequential append behavior reduces internal RMW cycles, and NAT indirection stabilizes update locality.
...
This 85\% reduction in tail latency under aged, degraded conditions indicates that reduced write amplification indirectly stabilizes latency behavior---GC pressure contributes to p99 variability, and endurance optimization can improve operational determinism. This latency improvement remains secondary supportive evidence rather than the paper's primary contribution; the central claim is write-amplification reduction, not latency optimization.
...
\begin{itemize}
    ...
    \item \textbf{JEDEC-Model Approximation:} The lifespan projection assumes uniform wear-leveling across the entire medium; early block failure, static wear imbalance, or retention leakage may cause premature failure before the analytically projected limit.
    ...
    \item \textbf{SD-Controller Quality Variance:} Consumer SD controllers exhibit significant quality variance across vendors; thermal degradation, power instability, and firmware bugs may alter effective endurance independently of host-side optimizations.
\end{itemize}
...
This study addressed a flash-endurance engineering challenge: the premature wear of low-cost storage under sustained embedded ML workloads. Rather than modifying proprietary controller firmware, the approach focuses on reshaping host I/O behavior to align with NAND physics.
