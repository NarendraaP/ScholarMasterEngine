...
Early engagement monitoring systems focused on facial expression recognition (FER) which associated facial landmarks with certain emotional categories \cite{b2}. Though useful in an affective computing context, this approach assumes that affect has a direct relationship to behavioral engagement.

The assumption fails in an educational setting, where pedagogical engagement takes place in a multi-dimensional space distinct from raw emotional valence \cite{b7}---a student may be paying close attention, despite a neutral-to-negative facial expression. Concentrated effort can visually resemble disengagement, and unimodal pipelines lack the contextual grounding necessary to differentiate between frustration that impedes learning and concentration that facilitates it.
...
Many multimodal approaches focus on prosody, such as pitch and speaking rate, \cite{b9} which are less prominent in a technically dense lecture delivered in a relatively monotone voice.
