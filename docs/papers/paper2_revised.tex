...
% ==========================================
\begin{abstract}
Automated engagement models frequently encounter a structural ambiguity in context-dependent classification settings. Facial expressions commonly associated with negative affect—such as furrowed brows or sustained gaze—also appear during periods of intense cognitive concentration. As instructional complexity increases, unimodal vision-based models therefore risk systematically misinterpreting focused effort as disengagement. Conventional cost-sensitive learning approaches attempt to compensate for this asymmetry during training, yet the reasoning behind individual predictions often remains difficult to interpret.

To address this ambiguity, this study proposes a contextual probabilistic interpretation layer that explicitly models the imbalance between observed facial valence and instructional context. The framework integrates facial valence estimation with schedule metadata and semantic density. During segments characterized by high instructional load, the model re-weights the posterior probability of engagement. We describe this recurring misinterpretation as a ``Valence Discrepancy'' and treat lexical density as a context-conditioned Bayesian prior.
...
Most end-to-end deep learning models internalize this trade-off implicitly within the loss function. Here, we instead make the asymmetry explicit. The proposed probabilistic framework makes error priorities explicit through structured priors and temporal smoothing, allowing instructors to audit how contextual signals influence decisions.

This study isolates the logical and mathematical formulation of the probabilistic interpretation layer, independent of identity resolution or distributed deployment concerns.
...
Empirical validation requires controlled observation of the engagement ambiguity described above. To evaluate the probabilistic logic without exposing real student data during early prototyping, we constructed a controlled dataset referred to as Sim-Class-24.

\subsection{Simulation Protocol}
The dataset consists of 24 staged evaluation sessions, each lasting approximately 60 minutes, yielding $N = 1,440$ evaluated temporal windows. Rather than collecting naturalistic evaluation recordings at this stage, we scripted controlled behavioral conditions to isolate the logic layer under reproducible circumstances.
...
% ==========================================
\section{Limitations}
All quantitative results are derived from the scripted Sim-Class-24 dataset. Although environmental noise was injected, real-world evaluation settings introduce additional variability not captured in simulation.
...
Additionally, lexical density depends on domain-specific keywords. Conceptually dense but lexically simple explanations may not trigger the high-load prior. Field validation across diverse context-dependent classification settings remains necessary before deployment.
...
The results suggest that relatively lightweight probabilistic layers, when grounded in pedagogical theory and probabilistic reasoning, can meaningfully shift the error balance while preserving interpretability. Context-conditioned priors therefore provide a practical mechanism for aligning engagement analytics with pedagogical realities while preserving model transparency.
