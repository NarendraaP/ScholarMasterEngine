...
\subsection{Event Tuple Definition (FIX 5 - APPLIED)}
\begin{equation}
e_i = \langle ID_{hash}, Loc_{zone}, Time_{stamp}, Conf_{score} \rangle
\end{equation}
Where $ID_{hash}$ is a cryptographic hash (e.g., SHA-256) of the student identity, $Loc_{zone}$ is the discrete zone ID, and $Conf_{score}$ is the sensor confidence (0.0 to 1.0). While hashing prevents direct identifier readability, it does not provide anonymity against adversaries with enrollment databases who can precompute hash mappings. True anonymity would require k-anonymity \cite{b15} or differential privacy \cite{b16} techniques beyond this logic layer.
...
This is a dynamic constraint involving the history of the entity. This acts as our primary "Anti-Spoofing" mechanism. Given two consecutive events $e_{t1}$ and $e_{t2}$ for the same ID:
...
\subsection{Data Management Implementation (FIX 6 - APPLIED)}
...
\subsection{Constraint Solver Implementation (FIX 7 - APPLIED)}
...
\subsection{Synthetic Dataset Generation (FIX 8 - APPLIED)}
...
\subsection{Hardware Setup (FIX 9 - APPLIED)}
...
% --- FIGURE 2: LATENCY GRAPH (FIX 10 - APPLIED) ---
\begin{figure}[htbp]
\centering
\begin{tikzpicture}
\begin{axis}[
...
\end{axis}
\end{tikzpicture}
\caption{Projected constraint validation latency based on $O(1)$ Redis lookup complexity (Section VI.3) and Python function call overhead. Curve represents expected behavior under synthetic event replay. Solid empirical validation would require production deployment with sustained traffic.}
\label{fig:latency}
\end{figure}

\subsection{Expected Performance Characteristics (FIX 11 - APPLIED)}
...
\begin{table}[htbp]
\caption{Expected Constraint Validation Correctness Under Synthetic Test Scenarios}
\begin{center}
\begin{tabular}{lccc}
\toprule
...
\subsection{Ethical and Institutional Considerations (FIX 13 - APPLIED)}
...
Currently, the system uses static rules. Future work will implement Federated Learning to allow edge devices to learn "Normal" transition patterns without sending raw data to the central server, improving privacy.
...
[Online]. Available: https://redis.io/topics/lru-cache.

\bibitem{b21} 
J. Kreps, N. Narkhede, and J. Rao, 
"Kafka: A Distributed Messaging System for Log Processing," 
in \textit{Proceedings of the NetDB}, 2011, pp. 1-7.
