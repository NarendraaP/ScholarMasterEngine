...
Monitoring technologies increasingly operate as infrastructural components within modern educational environments. These systems automate administrative tasks such as attendance tracking, classroom analytics, and safety monitoring, often relying on sensor networks that remain largely invisible to the individuals they observe. Bentham's Panopticon describes an architectural model in which individuals regulate their behavior because they know they might be observed at any moment, even if they cannot confirm they are currently being watched \cite{b1}. Although originally proposed as an optimized prison design in the eighteenth century, the concept has evolved into a widely cited metaphor for modern surveillance societies. 

Despite their functional utility, such infrastructures can create persistent uncertainty regarding the scope and intent of observation. Contemporary smart-campus deployments sometimes replicate aspects of this panoptic dynamic, often unintentionally. Cameras and automated monitoring systems frequently operate as opaque technological artifacts whose capabilities and retention policies remain largely invisible to the people being observed. When confronted with such opacity, students often assume worst-case monitoring scenarios. They operate under the assumption that their physical movements, facial expressions, and private interactions are continuously recorded, analyzed, and permanently archived. This perception can generate what surveillance scholars describe as a Chilling Effect, in which individuals suppress legitimate behavior due to the perceived risk of observation \cite{b7}.

Modern monitoring architectures often incorporate extensive privacy-preserving mechanisms, including data minimization pipelines and automated deletion policies. However, a technical safeguard that remains invisible to users may have limited social relevance. If individuals cannot see or independently verify that safeguards exist, they often default to the assumption that maximal surveillance is occurring, regardless of the institution's stated privacy policy.

A critical sociotechnical question therefore emerges: how does the visibility of these safeguards influence user trust? To investigate this question, the study adopts the framing of Automated Stewardship \cite{b13}. In contrast to traditional surveillance paradigms that emphasize maximal observational capability and hidden analytics, stewardship frames monitoring technologies as bounded systems designed to serve specific, transparent institutional purposes while visibly demonstrating their own limitations to the subjects being monitored. 

We designed a semester-long longitudinal field study to evaluate whether visible transparency mechanisms---such as real-time skeletal abstraction displays and user-accessible audit dashboards---can fundamentally alter student perceptions of monitoring technologies. Rather than modifying the underlying sensing infrastructure or the mathematical inference models themselves, the intervention focused purely on making the deployed system's existing operational constraints perceptible to the student body. In this framing, the monitoring system is not positioned as an invisible, omniscient observer, but rather as a visible participant subject to clear accountability and transparency rules.
...
Contemporary digital infrastructures increasingly transform behavioral data into predictive analytic resources \cite{b10}. Zuboff's analysis of Surveillance Capitalism describes how modern digital systems increasingly extract behavioral data from everyday human activities to generate predictive, commodified value. Within the domain of education, this dynamic appears in learning analytics platforms that track granular engagement metrics, behavioral patterns, and emotional signals to build predictive student models. 

Slade and Prinsloo \cite{b11} argue that such aggressively data-driven educational models risk reducing complex students to mere behavioral datasets while largely neglecting the ethical implications of the tracking itself. Similarly, Williamson \cite{b12} describes the emergence of ``psycho-informatics'' within EdTech platforms, where intimate emotional states are inferred from ambient behavioral signals. 

Privacy-centered deployments attempt to counter this trajectory by emphasizing stronger data minimization practices and operational transparency, principles closely aligned with Cavoukian's Privacy by Design framework \cite{b13}. Rather than obscuring the observation process to extract more organic behavior, stewardship models attempt to reveal operational constraints to foster a cooperative environment.
...
Furthermore, a Cohen's d effect size analysis yielded $d=1.8$ (95\% CI $[1.62, 1.98]$) for the Understanding of Data metric, consistent with a large effect size interpretation associated with the transparency intervention. A regression-based mediation analysis (Indirect effect = $0.41$, $95\%$ CI $[0.32, 0.52]$) suggests that reduced surveillance anxiety partially mediates the relationship between visual transparency and trust formation. Notably, 7\% of participants showed no change or slight decreases in Trust scores, suggesting heterogeneous adaptation patterns.
...
This disparity highlights an important design implication: perceptible privacy signals carry vastly greater psychological weight than invisible technical guarantees. Users must be able to visually infer the system's operational limits in order to feel comfortable with its presence.
...
The observed data are consistent with a mediated trust formation hypothesis in which visible abstraction reduces affective anxiety ($A = f_1(V)$), which subsequently acts as a necessary bridge to enable cognitive verification ($C = f_2(A, V)$), ultimately leading to system acceptance ($S = f_3(C)$).
...
Our observational data tightly aligns with this model: Affective Trust appears to function as a precursor for Cognitive Trust. Students who remained highly anxious in Phase 1 were generally uninterested in auditing complex logs; they simply expressed a desire for the system to be removed entirely. Once affective security was established by the visual abstractions in Phase 2, engagement with the cognitive audit features increased.
...
This longitudinal study has examined how visible transparency mechanisms influence trust in automated monitoring systems within educational environments. Following the transition from an opaque deployment model to one incorporating skeletal abstraction displays and student-accessible audit dashboards, mean Trust scores increased dramatically from 2.4 to 4.1 on a five-point scale.

The results suggest that privacy-preserving backend mechanisms such as encrypted storage and automated deletion policies may not strongly influence user perception when they remain hidden from view. When operational constraints are made clearly observable through interface design, users appear significantly more willing to interpret the system as bounded, accountable, and legitimate.

Institutional monitoring systems that visibly communicate their operational constraints may therefore achieve higher trust than systems relying only on invisible backend safeguards.
