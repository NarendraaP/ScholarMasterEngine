...
To support reproducibility, the source code for the inference engine and context fusion logic is made available as an open-source artifact \cite{scholarmaster_repo}. The repository contains the implementation of the fusion algorithms and configuration files used in the experiments described herein.
...
\caption{Qualitative Inference Trace: A snapshot of the decision logic, showing the system detecting a semantic keyword (``Integral'') and subsequently re-weighting the interpretation of a negative facial expression.}
\label{fig:log}
\end{figure}
