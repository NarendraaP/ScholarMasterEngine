...
Open-set identification introduces an additional challenge beyond nearest-neighbor retrieval. In high-dimensional embedding spaces, nearest-neighbor similarity is not informative in sparse regions. A single high cosine similarity does not imply a correct identification due to potential hubness in the embedding space. Open-set retrieval requires stability of cluster centers rather than individual similarity peaks.

In order to increase robustness, we propose the Local Density Consistency Criterion (LDCC). The acceptance of a retrieval result is based on both the confidence induced by the single vector query and its local embedding topology. In addition to determining whether the candidate belongs to the same cluster as the query set, we also take into account that a probe should be inside a region which has a similar density. Thus, a probe will be accepted only when both conditions are satisfied:
\begin{equation}
...
The first condition guarantees enough angular margin, and the second one ensures that the neighborhood forms a compact cluster. Although LDCC makes boundaries clearer, the open-set stability is still probabilistic and topological, depending on the separation of the embedding space.
