...
% TITLE
% ==========================================
\title{Jitter-Bounded State Isolation for Low-Latency Stateful Stream Evaluation}
...
We further introduce the concept of \emph{Jitter-Bounded State Isolation}, which provides guarantees on both ordered evaluation correctness and bounded memory growth under configurable jitter constraints. Microbenchmark experiments on a commodity cluster demonstrate near-linear throughput scaling under Zipfian workloads, sustaining more than 50,000 events per second with tightly controlled tail latency. The Lazy Migration protocol reduces P99 state transfer latency to approximately 1.9 ms during rebalancing. These results demonstrate that carefully engineered state management and migration strategies can sustain low-latency evaluation. This remains true even as cluster topology evolves dynamically.
\end{abstract}
...
This approach seeks to balance correctness with disciplined memory usage: by retaining only a finite temporal window per entity, the engine avoids unbounded accumulation of historical state that would otherwise accompany fully asynchronous assumptions \cite{b26}.

\subsection{Design Constraints}
Designing a model capable of meeting these constraints requires coordinated solutions across routing, state storage, and cluster elasticity without relying on heavy external infrastructure:
\begin{enumerate}
    ...
    \item \textbf{Seamless Elasticity:} Rebalancing the massive in-memory state across nodes during cluster scale-up or scale-down without incurring "stop-the-world" latency pauses \cite{b21, b23}.
...
The system consists of stateless ingress gateways $\mathcal{G}$ and a collection of stateful worker nodes $\mathcal{N} = \{N_1, N_2, \dots, N_k\}$. 
\begin{itemize}
    \item \textbf{Asynchrony:} We assume a fully asynchronous network model. There are no theoretical bounds on message transmission delays or relative processor speeds between any two nodes in $\mathcal{N}$.
    \item \textbf{Failure Model:} Nodes operate under a \textit{Crash-Recovery} failure model. A stateful worker node may halt execution unexpectedly and lose its entire volatile memory state ($\sigma_i$), but may later recover, reboot, and rejoin the cluster. We assume Byzantine failures do not occur at the internal node level.
...
Due to the nature of distributed sensing, we define network jitter as the temporal inversion between occurrence and arrival: $J(e) = \tau_{arr} - \tau_{occ}$. While the network is formally asynchronous, we assume the maximum acceptable delay for a valid event is bounded by a strict system parameter $\Delta_{jitter}$. Any event where $J(e) > \Delta_{jitter}$ is classified as a severe straggler and is dropped by the system's ingestion layer. Consequently, the system therefore operates under a partially synchronous assumption bounded by the configurable $\Delta_{jitter}$ parameter \cite{b26}.

\subsection{State Partitioning Model (Shared-Nothing)}
To avoid the scalability limitations of centralized databases, the system adheres to a strict Shared-Nothing model \cite{b2, b22}. The global state vector $\Sigma$, which maps every entity $\epsilon$ to its evaluated temporal history, must be disjointly partitioned. 

Let $\mathcal{R}$ be a continuous, circular hash ring defined on the integer space $[0, 2^{32}-1]$. 

The mapping function $\Phi: \mathcal{E} \to \mathcal{N}$ uniquely assigns the authoritative state of entity $\epsilon$ to exactly one live node $N_i$ at any given logical time. This strict affinity routing ensures that $\Sigma = \bigcup_{i=1}^{|\mathcal{N}|} \sigma_i$ and that the intersection $\sigma_i \cap \sigma_j = \emptyset$ for any $i \neq j$. Consequently, any evaluation logic applied to $\epsilon$ requires zero cross-node communication.
...
The functionality of the stream engine emerges from the interaction of several distributed algorithms responsible for routing, temporal ordering, elasticity, and durability.

\subsection{Consistent Hashing and Gossip-Based Routing}

Ensuring state locality across a distributed cluster requires a deterministic routing mechanism that maps events belonging to the same entity to a single processing node without relying on a centralized coordination service (like Apache ZooKeeper). The ingress gateways $\mathcal{G}$ apply Consistent Hashing with Virtual Nodes \cite{b3, b28}. To ensure uniform load distribution even if the hardware cluster is small, each physical node $N_i$ is mapped to $V$ virtual nodes distributed randomly across the hash ring $\mathcal{R}$. 

The nodes independently maintain an eventually consistent view of the cluster topology using an assumed membership service such as a SWIM-based gossip protocol \cite{b7, b24}. In this protocol, each node periodically sends direct UDP ping messages to a randomly selected subset of peers. If a direct ping times out, the node requests indirect probes via intermediary peers. If all indirect probes fail, the target peer is officially marked as dead. The detecting node updates its local hash ring $\mathcal{R}$ and disseminates this topology change via epidemic propagation. 
...
As shown in Algorithm \ref{alg:window}, the system effectively defers finality. By continuously maintaining the $\Delta_{jitter}$ window and garbage collecting stale data, the algorithm operates identically to the "Watermark" concept established in the Google MillWheel Dataflow model \cite{b13}, ensuring memory growth remains bounded under configured jitter assumptions while safely accommodating network delays.

\subsection{Lazy State Migration Protocol}

Cluster elasticity introduces additional complexity since state partitions must be redistributed whenever nodes join or leave the system. Topology changes occur frequently in elastic clusters. When a new node joins, portions of the hash space must be reassigned. Traditional approaches often migrate entire key ranges immediately, often introducing noticeable latency spikes as donor nodes stall to transfer state \cite{b32}.

We instead adopt a Lazy State Migration strategy \cite{b23}. Instead of preemptively copying all reassigned state, the system transfers historical data only when a newly assigned entity becomes active. Inactive entities remain unmoved.

\begin{algorithm}[h]
\caption{Lazy State Migration (On Node $N_{new}$)}
\begin{algorithmic}[1]
...
As detailed in Algorithm \ref{alg:migration}, the newly provisioned node $N_{new}$ assumes routing responsibility immediately. The state is only migrated over the network at the exact microsecond an active entity triggers a new event.
...
Fault tolerance in a distributed stream processor requires periodic persistence of in-memory state. To tolerate crashes, the system must periodically serialize its state to durable storage. We utilize an abstract asynchronous snapshot mechanism adapted from the principles of Chandy-Lamport \cite{b4, b25}, conceptualized here via OS-level Copy-On-Write (COW) semantics \cite{b30}. Runtime execution semantics and process lifecycle management are treated separately.
...
The system maintains what we describe as a Serialized Ownership Invariant: at any moment, authoritative mutable state for a given entity resides on at most one node. During lazy migration, the previous owner serves the historical state exactly once and immediately marks it read-only before deletion. This prevents concurrent mutation across nodes while preserving continuity \cite{b29}.
...
Under network partitions, if the gossip protocol assigns an entity to $N_{new}$, but $N_{new}$ cannot retrieve the entity's history because $N_{old}$ is unreachable, the system rejects evaluation under incomplete causal history. Initializing an empty state without history would violate application-defined validation routines. Instead, the system prioritizes state consistency over availability, dropping the event or returning a partition error. This behavior explicitly sacrifices availability to preserve state continuity under partition \cite{b14}, though this behavior is configurable for use cases prioritizing availability.
...
\begin{itemize}
    ...
    \item \textbf{Migration Evaluation:} If the incoming event triggers the Lazy Migration protocol, the system incurs the base evaluation time plus exactly one synchronous network Round Trip Time (RTT). Total time: $\mathbf{O(\log K + \text{RTT}_{net})}$.
...
Let $N_{active}$ be the number of unique entities actively assigned to a specific physical node. Each entity maintains a Skip List of size $\le K$. The total volatile memory footprint per node is bounded as a function of $\Delta_{jitter}$ and active entity count:
\begin{equation}
...
\begin{itemize}
    \item \textbf{Gossip Protocols:} Each node sends background UDP ping messages to its peers. The global network overhead is amortized $\mathbf{O(1)}$ per node per interval, which remains highly scalable and consumes negligible bandwidth even in clusters containing hundreds of nodes.
...
% ==========================================
\section{Engine Microbenchmarks}
...
\textbf{Scope of Evaluation:} This section evaluates the isolated algorithmic components of the distributed engine via targeted microbenchmarks. Full-system integrated stress testing and application-layer database persistence are explicitly out of scope for this algorithmic evaluation. 
}}
\end{center}

\subsection{Implementation and Setup}
Experiments were conducted on a 10-node commodity computing cluster. The core algorithms were implemented utilizing Lock-Free Skip Lists and asynchronous I/O APIs. To bypass external I/O bottlenecks and purely evaluate the algorithmic logic, a local workload generator was deployed. It produced a synthetic stream of encoded events modeling 1,000,000 active entities injected directly into the ingress gateways. The workload followed a Zipfian distribution, mirroring real-world conditions where a small subset of entities generates the majority of the events. Results are averaged over 5 independent load runs, with standard deviation $<3.2\%$.

\subsection{Throughput and Horizontal Scaling}
To validate the $O(1)$ node-isolation properties of the shared-nothing architecture, we measured the maximum sustained throughput of the cluster before the 99th percentile (P99) latency exceeded a strict 10ms threshold.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}
\begin{axis}[
    title={Engine Throughput vs. Shard Count},
    xlabel={Number of Physical Nodes ($|\mathcal{N}|$)},
...
\end{axis}
\end{tikzpicture}
\caption{Throughput scalability analysis under a Zipfian workload. The distributed stream processor achieves near-ideal linear scaling ($R^2=0.985$). The absence of global distributed locks allows the cluster to seamlessly process 51,000 events per second at 10 nodes.}
\label{fig:throughput}
\end{figure}

As illustrated in Figure \ref{fig:throughput}, throughput increases proportionally with shard count under the evaluated synthetic workload. While perfect linear scaling is unattainable in practice, the observed $R^2=0.985$ indicates that the absence of centralized coordination points allows the system to approach the ideal line under moderate cluster sizes.

\subsection{State Migration Microbenchmark}
We induced a deliberate cluster rebalancing event by manually provisioning and adding an 11th node to the cluster under a constant, heavy load of 25,000 events per second. We evaluated the pure networking and memory transfer latency of the event stream, comparing our proposed Lazy Migration protocol against a baseline Eager Migration methodology.

During induced rebalancing, eager migration produced substantial latency spikes due to bulk transfer blocking (recording a P99 latency of 420ms). By contrast, Lazy Migration deferred transfers to active entities only, keeping P99 latency below 1.9 ms (standard deviation $<3.2\%$) under controlled intra-cluster conditions. While these measurements exclude cross-region RTT variability, they demonstrate the intended smoothing effect of on-demand migration.

\subsection{Snapshot CPU and Memory Overhead}
We profiled the CPU and memory impact of the snapshotting algorithm on a fully loaded node processing 5,000 events per second. The abstract snapshot operation blocked the main processing thread for $<1.5$ms. During the subsequent 4.2 seconds required for the child process to serialize the state to durable storage, the parent process memory grew by only 18MB (measured with approximately 2.5 million in-memory entries, representing $\approx 1.2$ GB of Resident Set Size (RSS)). This minimal growth is attributed to the highly efficient page fault management of the OS Copy-On-Write mechanism.
...
By formalizing the concept of Jitter-Bounded State Isolation, the system provides bounded guarantees on both ordering correctness and memory growth within configurable jitter parameters. Microbenchmark experiments suggest that the architecture scales proportionally under skewed workloads and that lazy migration significantly reduces disruptive latency during cluster rebalancing events.
