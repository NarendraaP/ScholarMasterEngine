...
\begin{IEEEkeywords}
Augmented Reality, Human-Computer Interaction, Cognitive Load, Situational Awareness, Fitts's Law, Ergonomics, Edge Visualization, Smart Campus.
\end{IEEEkeywords}
...
Monitoring infrastructures deployed across modern campuses increasingly rely on automated analytics to identify operational anomalies. While detection algorithms scale efficiently, human operators must still interpret these alerts and determine appropriate responses. On a large, modern campus, dozens or even hundreds of concurrent events may be generated across multiple buildings simultaneously. While backend analytics can scale efficiently to handle this, the human interface layer does not scale gracefully \cite{b1, b28}.

In practice, the limiting factor in these systems is rarely anomaly detection accuracy but rather the human interpretation pipeline. Most monitoring environments present alerts through conventional dashboard interfaces. These typically consist of scrolling logs and static maps. Over extended monitoring periods, navigating this format introduces severe mental fatigue and slows critical interpretation \cite{b2}. The principal bottleneck is often not the presence or speed of the data itself; rather, it is the mental effort required to convert symbolic text into spatially actionable understanding \cite{b29}.

Interpreting these alerts requires operators to mentally translate symbolic notifications into real-world spatial locations. Consider a typical alert prompt: \textit{``Acoustic Anomaly: Stairwell North-3.''} 
Upon receiving this notification, the operator must perform a sequence of mental transformations. First, they must read and comprehend the text. Second, they map that text string to a mental 3D blueprint of the physical building. Third, they must calculate a navigable physical route from their current location to the target. Finally, they must visually identify the specific door or corridor upon arrival. 

This additional cognitive translation step introduces delays that are largely avoidable, particularly under time-sensitive pressure. Even minor misinterpretations---such as turning down the wrong corridor---compound response times in multi-wing academic buildings \cite{b3, b30}.

This work therefore explores whether spatial computing interfaces can remove the cognitive translation step entirely. Using mobile AR, we overlay contextual data precisely onto the operator's real-world camera view. Under this spatial computing paradigm, an anomaly is no longer just a text log on a screen; it becomes a physical indicator positioned directly over the relevant, real-world location \cite{b31}.

It is important to emphasize that this paper focuses strictly on the presentation layer. We do not propose new anomaly detection algorithms, nor do we evaluate the upstream infrastructure that generates events. This work evaluates AR strictly as a cognitive performance interface. It assumes the existence of valid, spatially-referenced event streams and seeks solely to optimize their human ingestion, exploring how spatial computing can alleviate the cognitive limitations imposed on traditional 2D dashboard operators \cite{b32}.
...
Augmented reality can be interpreted as a cognitive offloading mechanism that externalizes spatial reasoning tasks. Let the Time-to-Action (TTA) be defined as:
\begin{equation}
TTA = f(M + S)
\end{equation}
where $M$ represents the mental rotation and translation cost, and $S$ represents the spatial anchoring cost.

In traditional 2D dashboard environments, $M$ remains high while $S$ stays relatively low because the operator must mentally convert symbolic text alerts into physical 3D coordinates. In the AR paradigm, spatial anchoring is externally provided through visual overlays, leading to $M \downarrow$ and $S \uparrow$. Crucially, the increase in spatial anchoring cost is absorbed by the visual cortex rather than conscious working memory, producing an overall reduction in cognitive burden \cite{b25}.
...
Traditional text logs frequently succeed at Level 1 but fail at Level 2; operators perceive the alert but struggle to comprehend its physical spatial context quickly under stress. AR interfaces inherently support Levels 1 and 2 by placing the visual indicator directly on the physical target, completely bypassing the need for mental integration and immediately enabling Level 3 projection capabilities \cite{b5, b28}.
...
Mean performance improved from a baseline of 48.5 seconds in the dashboard condition to 28.1 seconds in the AR condition. The AR interface yielded a 42\% improvement in response speed. A paired t-test confirmed the statistical significance of this result ($t(19)=14.8, p<0.01$), with a very strong effect size ($d = 1.12$). 

Furthermore, the dashboard condition resulted in an average of 3.2 wrong turns per trial, compared to only 0.4 in the AR condition. Observational data indicated that most baseline errors involved incorrect corridor selection immediately following orientation changes (e.g., exiting a stairwell). The spatial alignment provided by the AR interface noticeably reduced this class of navigational error. Two trials in the AR condition experienced temporary SLAM relocalization loss in reflective corridor segments, requiring manual re-anchoring. These runs were retained in analysis to preserve ecological validity.
...
The AR interface offers a clear heads-up operational advantage: operators maintain continuous visual awareness of the environment while simultaneously receiving alerts \cite{b25}. This contrasts sharply with dashboard interaction, which forces dangerous, frequent downward glances while walking \cite{b29}.

However, sustained device elevation can cause shoulder fatigue, commonly known as Gorilla Arm Syndrome \cite{b22}. Two participants reported mild shoulder fatigue during the trials. For full-shift operations, handheld AR functions best as an intermittent verification tool rather than a continuous lens. Future form factors exploring lightweight, optical see-through AR glasses may mitigate this physical strain, provided hardware costs decrease sufficiently for broad academic deployment \cite{b10}.

\subsection{Environmental Constraints}
Tracking reliability undeniably decreases under specific environmental conditions, notably low lighting and highly reflective surfaces. To mitigate this, the client includes a fallback mechanism: when ambient light sensors detect insufficient illumination, the interface automatically transitions into a 2D radar mode to preserve navigational continuity \cite{b24}.
...
This study examined the human-interface bottleneck that arises when automated monitoring systems generate large volumes of alerts. By rendering abstract event data as spatially anchored AR visualizations, the interface produced 42\% faster response times and substantially fewer navigation errors, accompanied by measurable reductions in perceived cognitive load.

These results indicate that performance limitations in high-throughput monitoring environments often arise not from the underlying detection algorithms but from how information is presented to human operators. Interfaces that eliminate the need to translate symbolic alerts into spatial reasoning tasks may therefore significantly improve operational responsiveness.

Spatially anchored visualization therefore offers a promising strategy for improving human response efficiency in high-throughput monitoring environments.
