...
The main idea of this paper is that hierarchical policy-aware orchestration can provide deterministic and fault-contained coordination of heterogeneous edge AI modules operated within common constraints. Edge intelligent systems \cite{b20} and edge computing foundations \cite{b11, b38, b39} often feature multiple domain-specific AI modules hosted at the edge, running concurrently, for instance, biometric recognition, pose estimation, and speech understanding, with multimodal facial expression toolkits like AFFDEX \cite{b7}. While each can be optimized individually, their joint orchestration in a shared runtime environment introduces a distinct coordination problem \cite{b14, b21}.

\subsection{Architectural Fragility in Flat Multi-Model Pipelines}
When modules are tightly coupled in flat execution pipelines, localized crashes in one heavy perception model can easily propagate, causing cascading failure across the entire sensing system. Shared compute contention interferes with other wise independent modules, while cloud mediated orchestration introduces unpredictable network latency that fails to meet the demands of interactive modeling in smart campus environments \cite{b4, b14, b19}. Furthermore, naive always-on sensing waste computing resources by performing heavy duty processing when not needed. Orchestration destabilization is caused by topological factors rather than mere compute model inefficiency.
...
Dynamic pipeline adaptation for continuous video analytics has received extensive attention. Zhang et al. developed VideoStorm \cite{b33}, which dynamically adjusts frame rates and image resolutions to maximize query quality under resource constraints on GPU clusters. Jiang et al. introduced Chameleon \cite{b34}, periodic spatial-temporal profiling to dynamically adapt deep neural network configurations. Gujarathi et al. proposed Ekya \cite{b35}, balancing continuous retraining and inference scheduling on edge clusters. Xu investigated approximation and adaptive scaling \cite{b36} for streaming video analytics on mobile devices. While VideoStorm and Chameleon optimize throughput and query accuracy on unconstrained cloud/cluster environments, they assume always-on sensing and lack formal policy-driven suppression primitives required for privacy-constrained and energy-bounded cyber-physical deployments.

\subsection{Edge Multi-Model Orchestration and Microservice Systems}
Standard edge AI runtime engines, such as TensorFlow Lite \cite{b10, b31} and ONNX Runtime, focus on single-model tensor optimizations (quantization, pruning, operator fusion) but provide no native facilities for cross-module coordination, fault containment, or policy-aware rate governance. Microservice frameworks for edge analytics, such as Mainstream \cite{b21} and edge cloudlet systems \cite{b6, b23}, support concurrent pipeline execution through stem-sharing. Edge serving architectures such as Clipper \cite{b22} and hardware-level memory efficiency bounds \cite{b40} optimize execution throughput, but flat microservice architectures lack strict layer isolation: an uncontained memory leak or SIGSEGV fault in a heavy vision container cascades across the shared message broker, destabilizing concurrent lightweight pipelines \cite{b24, b18}.

\subsection{Policy-Aware Inference Governance and Privacy Gating}
Recent works highlight the necessity of privacy-by-design sensing boundaries in institutional environments \cite{b4, b27}. Systems such as Privid \cite{b27} and Vigil \cite{b37} enforce privacy at the query level by releasing bounded analytics. In the context of education and cyber-physical monitoring, indiscriminate continuous capture violates compliance doctrines. In contrast to after-the-fact data scrubbing, our hierarchical control plane composes the Compute Justification Model directly into the scheduling loop, suppressing perception activation through mathematical analysis based on physical context and policy state.
...
\begin{enumerate}
    \item \textbf{Boundary Confinement:} High fidelity perception data (e.g., raw video frames) must be confined to lower layers of abstraction to avoid unneeded exposure to upper layer business logic \cite{b27}.
    \item \textbf{Real-Time Responsiveness:} Cross-module coordination must operate within strict latency constraints to maintain systemic interactivity \cite{b21}.
    \item \textbf{Failure Tolerance:} Individual module faults should gracefully impact overall functionality, rather than halting the entire sensing system \cite{b24}.
\end{enumerate}
...
\end{equation}
By only receiving abstracted event tuples from $\vec{C}_{Perc \to Reas}$, the Reasoning and Governance layers are insulated from receiving raw sensory data. The use of typed vectors restricts the potential for cascading semantics such that higher layer modules only see abstractions bounded by their type. Orchestration isolation is made possible in this way.

\subsection{Layer Isolation Factor (LIF)}
To evaluate the quality of orchestration topology we define Layer Isolation Factor (LIF). Scored on per each fault injection scenario, LIF evaluates the isolation topology, not uptime probability. Given an isolated crash, is it cascading?
\begin{equation}
...
Let physical subjects be allowed to move with bounded maximum transit velocity $v_{\max} = 5.0\text{ m/s}$ within an observed optical detection corridor of depth $\Delta x$. In order to guarantee that no subject will traverse the critical interaction zone without being detected (zero state transition loss), the maximum allowable sampling period $T_s^*$ satisfies:
\begin{equation}
...
\begin{IEEEproof}
Let a subject enter the detection corridor at time $t_0$ and position $x(t_0) = 0$. In the worst-case velocity regime, $x(t) = v_{\max} (t - t_0)$. The transit time across a corridor length $\Delta x$ is $T_{\text{transit}} = \Delta x / v_{\max}$. Consistent with the Nyquist-Shannon sampling criteria for state transitions, at least two distinct observations within the corridor are required to observe consistent kinematic directionality and velocity vectors. Thus, $2 T_s^* \le T_{\text{transit}} \implies T_s^* \le \frac{\Delta x}{2 v_{\max}}$.
\end{IEEEproof}
...
\begin{itemize}
    \item \textbf{Super-Kinematic Velocity Boundary ($v > 5.0\text{ m/s}$):} In activities that are either athletic or vehicular reaching speeds beyond $v_{\max} = 5.0\text{ m/s}$, the sampling period bound in \eqref{eq:kinematic_bound} demands higher frame rates than what is available in the standard 30 FPS camera hardware.
    \item \textbf{PID Gain Calibration Sensitivity:} PID gain calibration ($K_p, K_i, K_d$) requires mapping to edge-memory buffer capacities; aggressive $K_i$ values may induce transient overshoot on multi-person ingress events.
    \item \textbf{Cold-Start Activation Latency:} Re-activation of dormant deep neural networks from low power standby states incurs 150--350ms warm-up latency on embedded GPU accelerators.
    \item \textbf{Unscheduled Event Divergence:} Off-schedule classroom interaction events occurring between pedagogical phases may experience delayed capture until heuristic trigger thresholds are reached.
\end{itemize}
...
This paper presented a novel hierarchical edge control-plane architecture for policy-aware, fault-contained multi-module AI orchestration. The proposed approach formalizes Inference Rate Governance through kinematic-coupled velocity bounds and Lyapunov-stable PID queue regulation, achieving 72.4\% reduction in inference cycles while maintaining 98.7\% state capture fidelity and 95\% Layer Isolation Factor under fault injection. The control-plane paradigm eliminates cascading multi-model failures and enforces strict privacy-by-design compliance across edge cyber-physical deployments.
...
\bibitem{b38} M. Satyanarayanan, ``The Emergence of Edge Computing,'' \textit{Computer}, vol. 50, no. 1, pp. 30--39, 2017.
...
\bibitem{b7} D. McDuff et al., ``AFFDEX SDK: A Cross-Platform Real-Time Multi-Face Expression Recognition Toolkit,'' in \textit{ACM CHI Extended Abstracts}, 2016, pp. 3723--3726.
...
\bibitem{b36} R. Xu, ``Approximation for streaming video analytics on mobile devices,'' Doctoral dissertation, Purdue University, 2021.
...
\bibitem{b22} D. Crankshaw et al., ``Clipper: A Low-Latency Online Prediction Serving System,'' in \textit{14th USENIX Symposium on Networked Systems Design and Implementation (NSDI 17)}, 2017, pp. 613--627.

\bibitem{b40} P. Narendra et al., ``Memory-Bound Edge Efficiency Envelope (MBEEE): A Hardware-Level Analytical Model,'' \textit{Journal for Basic Sciences}, vol. 26, no. 5, pp. 31--37, 2026.
...
\bibitem{b37} T. Zhang, A. Chowdhery, P. Bahl, K. Jamieson, and S. Banerjee, ``The Design and Implementation of a Wireless Video Surveillance System,'' in \textit{Proc. 21st Annual International Conference on Mobile Computing and Networking (MobiCom)}, 2015, pp. 426--438.
