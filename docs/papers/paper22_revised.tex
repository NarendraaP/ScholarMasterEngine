...
Autonomous cyber-physical vision systems deployed on embedded edge hardware operate under harsh, dynamic physical environments characterized by variable illumination, camera defocus, motion blur, and unexpected out-of-distribution objects \cite{hendrycks2019benchmarking, dodge2016understanding}. Standard deep neural network (DNN) classifiers normalize their final layer via the softmax activation function:
\begin{equation}
...
\end{split}
\end{equation}
for any constant $c \in \mathbb{R}$. Consequently, softmax normalization evaluates only \textit{relative} logit magnitudes rather than \textit{absolute} evidential support. When presented with an uninformative out-of-distribution observation or severe sensory corruption, an edge network activating arbitrary high-frequency filters can generate large logits that produce a maximum softmax confidence approaching $1.0$ \cite{guo2017calibration, nguyen2015deep}. Ordinary softmax confidence is therefore semantically blind to the distinction between \textit{aleatoric uncertainty} (inherent observational noise) and \textit{epistemic uncertainty} (lack of model knowledge or distributional shift) \cite{der2009aleatory, malinin2018predictive}.

In multi-tier cyber-physical cascades—such as the 5-layer ScholarMaster architecture encompassing Perception Integrity (Layer 1), Biometric Identity (Layer 2), Kinematic Tracking (Layer 3), Temporal Compliance (Layer 4), and Ledger Commits (Layer 5)—this semantic blindness triggers catastrophic failure compounding. An overconfident classification at Layer 1 passes corrupted feature vectors into Layer 2 ArcFace embedding extractors \cite{deng2019arcface}. These vectors cross Voronoi decision boundaries in FAISS-HNSW graph indices \cite{malkov2018efficient}, flipping discrete identity states in Layer 3 Kalman tracking filters. The resulting spurious trajectory transitions violate Linear Temporal Logic (LTL) schedule formulas in Layer 4 \cite{pnueli1977temporal}, committing erroneous infractions to immutable administrative ledgers. This systemic failure cascade exemplifies the Data Cascade phenomenon documented by Sambasivan et al. \cite{sambasivan2021everyone}, where $92\%$ of real-world AI deployment failures originate from uncontained upstream sensory corruptions rather than downstream algorithmic defects.
...
\begin{enumerate}
    ...
    \item \textbf{Calibration vs Uncertainty Estimation}: Post-hoc probability calibration (e.g., Temperature Scaling \cite{guo2017calibration}) adjusts confidence magnitudes on in-distribution data but preserves argmax rankings, leaving out-of-distribution samples overconfident.
    \item \textbf{OOD Scoring vs Deterministic Gating}: Conventional out-of-distribution scoring mechanisms (e.g., Energy-based scoring \cite{liu2020energy}) compute unbounded scalar scores that lack deterministic safety thresholds under dynamic sensor gain shifts.
    \item \textbf{Image Quality vs Semantic Uncertainty}: Spatial and Fourier optical metrics \cite{pech2000diatom, pertuz2013analysis} detect physical defocus blur but are blind to semantic distributional anomalies.
    \item \textbf{Perception Integrity Contract}: A multi-tier cyber-physical edge pipeline requires a formal, single-pass integrity contract that validates input payloads before downstream computation.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{First-Principles Dirichlet Variance Bounds}: We derive closed-form upper bounds on Dirichlet class probability variance ($\mathrm{Var}(p_k) \le \frac{1}{4(S+1)} < \frac{1}{4K}$) and prove strictly monotonic variance contraction under proportional evidence scaling.
    \item \textbf{Multi-Orthogonal Composite Risk Function}: We formulate a strictly bounded composite perception risk $R_p \in [0, 1]$ unifying subjective vacuity, multi-branch cross-agreement, frequency-domain blur, and normalized keypoint dispersion.
    \item \textbf{Sub-Millisecond Edge Calibration \& Gating}: We demonstrate that temperature scaling coupled with Dirichlet evidential gating reduces Expected Calibration Error ($\text{ECE}$) by $90.2\%$ down to $0.0412$ and achieves perfect out-of-distribution discrimination ($\text{AUROC} = 1.0000$, $\text{FPR95} = 0.0000$) within a deterministic $1.486\text{ ms}$ execution budget.
    \item \textbf{Fail-Closed Quarantine System \& Operating Bounds}: We define a deterministic fail-closed state transition system that intercepts corrupted observations at Layer 1, establishing the mathematical boundary conditions required to eliminate downstream error propagation.
\end{enumerate}

\section{Related Work \& Analytical 6-Paradigm Taxonomy}
\subsection{Uncertainty Quantification \& Edge Computational Limits}
Uncertainty quantification in deep neural networks has traditionally followed Bayesian formulations. Bayesian Neural Networks (BNNs) \cite{blundell2015weight, neal1995bayesian} place prior distributions over network weight tensors, optimizing the variational evidence lower bound (ELBO) during training. At inference time, BNNs estimate epistemic uncertainty by drawing $N \ge 20$ weight samples. Gal and Ghahramani \cite{gal2016dropout} developed Monte Carlo Dropout (MC-Dropout), casting test-time dropout as approximate variational inference over network architectures. Lakshminarayanan et al. \cite{lakshminarayanan2017simple} introduced Deep Ensembles, training $M$ randomized network instances and aggregating their output distributions to capture epistemic diversity.

While Deep Ensembles and MC-Dropout provide robust uncertainty estimates, they impose severe computational penalties. An ensemble of $M=5$ models requires $5\times$ FLOPs and parameter storage ($18.2\text{ ms}$ latency), while MC-Dropout with $N=20$ passes scales latency to $28.5\text{ ms}$. In streaming edge vision operating at $30\text{--}60\text{ FPS}$, the total per-frame budget is bounded by $16.6\text{--}33.3\text{ ms}$, with Layer-1 perception allocated a strict SLA budget of $<5.0\text{ ms}$. Multi-pass Bayesian sampling is therefore computationally prohibitive for real-time edge microcontrollers and embedded neural accelerators \cite{sandler2018mobilenetv2, howard2019searching}.

To overcome the multi-pass bottleneck, Sensoy et al. \cite{sensoy2018evidential} formulated Evidential Deep Learning (EDL), mapping network activations to non-negative evidence vectors that parameterize a Dirichlet prior over class multinomials in a single forward pass. Malinin and Gales \cite{malinin2018predictive} extended this concept via Prior Networks, training models on synthetic out-of-distribution sets to explicitly separate data noise from distributional shift. However, existing EDL methods lack formal closed-form variance bounding proofs under edge constraints and remain susceptible to optical noise producing spurious evidence activations.
...
Modern deep networks with high capacity are notoriously miscalibrated, producing overconfident mispredictions \cite{guo2017calibration}. Post-hoc calibration methods adjust logit scaling without altering network parameters. Platt scaling \cite{platt1999probabilistic} fits a sigmoid mapping to binary logits, while Temperature Scaling \cite{guo2017calibration} introduces a single scalar $T > 0$ to rescale multi-class logits: $\tilde{\mathbf{z}} = \mathbf{z} / T$. Non-parametric approaches, such as isotonic regression \cite{zadrozny2002transforming}, provide non-linear mapping but risk severe overfitting in small validation regimes. Crucially, Temperature Scaling is a strictly monotonic transformation that preserves the maximum logit index; while it reduces calibration error on in-distribution samples, it cannot identify out-of-distribution inputs.

For out-of-distribution (OOD) detection, Hendrycks and Gimpel \cite{hendrycks2016baseline} proposed Maximum Softmax Probability scoring. Liang et al. \cite{liang2017enhancing} developed ODIN using input perturbations, whereas Liu et al. \cite{liu2020energy} introduced energy-based OOD scoring based on the Helmholtz free energy:
\begin{equation}
...
\end{equation}
Furthermore, as total Dirichlet strength increases ($S \to \infty$), the variance contracts asymptotically to zero: $\lim_{S \to \infty} \mathrm{Var}(p_k) = 0$.
\end{theorem}
...
For any two distinct classes $i \ne j$, the covariance between $p_i$ and $p_j$ on the probability simplex is strictly negative:
\begin{equation}
...
\end{equation}
where the weights are strictly normalized $\sum w = 1.0$ ($w_u=0.35, w_d=0.25, w_b=0.25, w_k=0.15$). Because each component satisfies $u, d, B, D_{norm} \in [0, 1]$, the composite risk $R_p(\mathbf{x})$ is a convex combination of bounded metrics, formally guaranteeing $R_p(\mathbf{x}) \in [0, 1]$.
...
\begin{enumerate}
    \item \textbf{Perfect OOD Discrimination}: Layer-1 Perception Integrity achieves an $\text{AUROC}$ of $1.0000$ and an $\text{FPR95}$ of $0.0000$, completely separating clean in-distribution inputs from out-of-distribution anomalies.
    \item \textbf{Substantial ECE Reduction}: Temperature scaling reduces the Expected Calibration Error by $90.2\%$, from an uncalibrated $0.4218$ down to $0.0412$, while lowering the Brier score from $0.3842$ to $0.1793$.
    \item \textbf{Decisive Risk Separation}: The composite perception risk scales monotonically from $0.0421$ on clean frames to $0.8954$ under OOD corruption, yielding a risk separation margin of $\Delta R_p = 0.8533$ under an edge latency of $1.486\text{ ms}$ (well within the $5.0\text{ ms}$ SLA target).
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Softmax Overconfidence vs Dirichlet Evidence Mass}: Standard softmax normalizes relative logits, generating high confidence for unconstrained OOD activations. In contrast, Dirichlet EDL parameterizes total evidence $S$; when an OOD sample lacks learned feature activations, evidence remains zero ($\mathbf{e} \to \mathbf{0}$), driving $S \to K$ and epistemic uncertainty $u \to 1.0$.
    ...
    \item \textbf{Signal-Feature Orthogonality}: Frequency-domain Modified Laplacian and Fourier integrals detect phase and gradient disruptions directly from raw pixel arrays, ensuring that blurred or defocused frames are flagged even if the neural network produces anomalous activations.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{False Acceptance vs False Rejection Trade-off}: Setting $\tau_{risk} = 0.70$ achieves a False Acceptance Rate of $\text{FAR} = 0.0000$, ensuring zero corrupted frames enter downstream Layer 2/3 pipelines. The operational cost is a $21.6\%$ quarantine rate ($78.4\%$ fast-path pass rate), where borderline frames are safely diverted to secondary verification cascades rather than accepted unconditionally.
    \item \textbf{Scope of Empirical Generalization}: While these results validate complete OOD separation across the 2,000-frame benchmark suite, this empirical finding does \textbf{not} constitute an unprovable universal theorem guaranteeing zero error under arbitrary open-world sensor distributions or physical lens destruction.
\end{enumerate}
