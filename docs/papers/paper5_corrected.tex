...
% ==========================================
\begin{abstract}
The transition of deep learning workloads from cloud data centers to distributed edge environments is significantly constrained by the "Memory Wall"—the bandwidth and latency bottleneck inherent in traditional heterogeneous architectures that couple Central Processing Units (CPUs) with discrete Graphics Processing Units (GPUs) \cite{b2}. In continuously active edge inference workloads, the energetic cost of data movement across the Peripheral Component Interconnect Express (PCIe) bus often exceeds the computational cost of inference itself \cite{b8}. This paper presents an architectural evaluation comparing Unified Memory Architectures (UMA), using the Apple M2 System-on-Chip (SoC) as an exemplar of the UMA class, against traditional discrete x86-CUDA architectures. By leveraging unified memory semantics native to modern integrated SoCs, we eliminate the redundant frame buffering inherent in PCIe-based discrete architectures. High-level ML frameworks (PyTorch MPS, CoreML) abstract kernel-level zero-copy primitives, enabling efficient memory sharing between CPU and Neural Engine subsystems. Architectural analysis and performance modeling suggest that UMA platforms can achieve up to a modeled 5.4x improvement in Performance-Per-Watt (PPW) compared to discrete baselines, sustaining 33.1 FPS at under 15 Watts. Furthermore, thermodynamic modeling utilizing the Arrhenius equation projects a theoretical MTTF improvement factor of >3x under continuous operation assumptions. Finally, we model the carbon intensity of operation, projecting that UMA deployment is projected to reduce the lifecycle carbon footprint by up to 68\% under sustained inference assumptions \cite{b12}. All efficiency, reliability, and sustainability improvements are derived from architectural modeling and reference profiling data, not from exhaustive empirical stress testing.
\end{abstract}
...
Biometric recognition (Paper 1), engagement inference (Paper 2), privacy-preserving sensing (Paper 3), spatiotemporal compliance (Paper 4), system orchestration, application-level deployment, or institutional validation. Application-level validation of these architectural bounds is reported in Paper 1.
...
Recent surveys of AI accelerators demonstrate the viability of UMA platforms for bandwidth-bound workloads \cite{b6}. Our work extends this evaluation to the domain of continuous streaming computer vision.
...
Equivalent zero-copy primitives exist in other UMA platforms (e.g., shared virtual memory in AMD APUs and coherent shared memory in ARM SoCs); IOSurface is shown solely as a concrete illustrative example. This architectural primitive ensures that latency measurements are purely computational, isolating hardware performance from software overhead \cite{b24}.
...
To ensure reproducibility, we explicitly detail the software versions used in Table I. We utilized PyTorch \cite{b13} for model definition and TVM \cite{b9} for optimization. The discrete system utilized TensorFlow \cite{b40} for comparison.
...
\textbf{Data Provenance Declaration:} All latency and energy figures presented in this paper (including Table III and efficiency ratios) are analytically derived architectural bounds or theoretical projections unless explicitly marked as empirical measurement.
}}
\end{center}
...
\end{center}
\vspace{0.2cm}
\textit{*Latency Analysis Methodology: Bus transfer values derived from PCIe Gen 3 x16 bandwidth specifications ($\approx$16 GB/s effective throughput) and typical frame buffer transfer sizes (1920×1080 RGB, $\approx$6.2 MB per frame). UMA memory access latencies reflect LPDDR5 unified memory access characteristics. Inference times based on PyTorch MPS profiling with standard dense computer vision workloads. Values represent architectural performance bounds rather than empirical stress-test measurements.}
\label{tab:latency}
\end{table}
...
Table IV shows the trade-off between numerical precision and hardware throughput. We observed that FP16 offers the optimal balance, doubling throughput compared to FP32 by effectively halving memory bandwidth pressure. Techniques such as Deep Residual Learning \cite{b41} were used to maintain tensor stability. We also referenced survey data from Gholami et al. \cite{b38} and whitepapers by Krishnamoorthi \cite{b39} and Gupta et al. \cite{b23} to validate our quantization strategy.

% --- TABLE IV: QUANTIZATION ---
\begin{table}[htbp]
\caption{Architectural Impact of Quantization on Throughput (Representative Workloads)}
\begin{center}
\begin{tabular}{lcc}
\toprule
\textbf{Precision} & \textbf{Model Size} & \textbf{FPS} \\
\midrule
FP32 (Baseline) & 98 MB & 14.2 \\
\textbf{FP16 (Ours)} & \textbf{49 MB} & \textbf{33.1} \\
INT8 & 25 MB & 41.5 \\
\bottomrule
\end{tabular}
\end{center}
...
\textsuperscript{*}\textit{Note: FPS values from PyTorch MPS profiling (UMA) and TensorRT benchmarking (discrete) under standard continuous vision workloads. Power values from vendor TDP specifications and \texttt{powermetrics}/\texttt{nvidia-smi} vendor-exposed counters. Efficiency calculation represents architectural bounds under reference workloads, not sustained deployment measurements.}
