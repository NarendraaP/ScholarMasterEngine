...
This work provides an empirical analysis of algorithmic biometric retrieval under tight latency constraints. We demonstrate graph-based sublinear retrieval achieving 0.419ms mean operational latency via HNSW at the 100K scale. Furthermore, we conduct open-set stress testing showing 100\% unknown rejection exclusively under fully synthetic separation conditions at a 20\% injection rate. Finally, we validate sustained streaming capability in a single-face gate scenario, functioning entirely without cloud dependency.
...
\begin{itemize}
    ...
    \item \textbf{Hardware Benchmarking:} Hardware selection rationale is analyzed in a companion architectural evaluation. This paper focuses strictly on the algorithmic retrieval performance of the system.
...
Related work by the author has examined general edge inference paradigms. A separate study investigated acoustic anomaly detection using localized processing. These studies are orthogonal to the present work, which concentrates solely on biometric vector representation and indexing algorithms.
...
Hardware selection rationale is analyzed in a companion architectural evaluation. The pipeline described here serves as the algorithmic implementation for biometric retrieval, enabling the sub-millisecond latencies reported in our computational logic results over the dataset.
...
\hline
\multicolumn{4}{l}{\textit{Measurements based on standard reported execution times for algorithmic baselines.}} \\
\end{tabular}
\label{tab_compare}
\end{center}
\end{table}
...
By decoupling embedding quality from retrieval mechanics and strictly enforcing algorithmic efficiency, this work demonstrates that the 'retrieval gap' at institutional scale is solvable via advanced graph indexing techniques. The proposed HNSW-based retrieval strategy is purely algorithmic and fundamentally hardware-agnostic. 

We validate that large-scale, open-set identification is computationally feasible without cloud dependency, establishing a blueprint for autonomous, high-throughput academic analytics systems based on localized vector databases.
...
Each paper addresses a distinct technical algorithm or methodology—such as algorithmic retrieval scaling, privacy protocols, or participation analytics—and all related works are cross-referenced for full transparency.
