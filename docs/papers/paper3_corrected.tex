...
% Define \RETURN command for algorithmic package (Match Paper 1 & 2)
...
% TITLE
% ==========================================
% DELTA: Title changed to "Participation Sensing" to avoid overclaiming engagement
\title{Privacy-Preserving Academic Participation Sensing via Pose-Only Architectural Irreversibility
...
% ==========================================
\begin{abstract}
Quantifying student participation is a critical metric for pedagogical improvement, yet the deployment of high-resolution cameras in classrooms raises significant ethical and regulatory concerns. Traditional affect recognition systems, which rely on facial analysis, increasingly conflict with data protection frameworks such as GDPR due to the storage of personally identifiable biometric data. This paper proposes a solution to this privacy--utility trade-off through a privacy-irreversible, pose-only sensing architecture. Unlike conventional approaches that process RGB facial features, our system utilizes lightweight pose estimation to extract **anonymous** skeletal vector maps, immediately discarding pixel data within a strictly enforced Volatile Zone (RAM-only). We introduce \textbf{Architectural Irreversibility} as a formal system property where raw biometric data cannot propagate beyond the volatile boundary by design, not policy. By analyzing relative coordinate topology (e.g., wrist vs. ear elevation) and head orientation (Perspective-n-Point) using only sparse landmarks, the system detects **geometric participation events** without facial biometric processing or affect inference. Experimental validation demonstrates 97\% geometric event detection accuracy in staged scenarios, while structurally preventing biometric persistence, retrospective surveillance, and pixel-space reconstruction attacks (gait-based re-identification remains a residual risk; see Section IX.A). We explicitly clarify that architectural irreversibility applies to biometric identity reconstruction, not to behavioral correlation conducted under lawful institutional consent. To our knowledge, this represents a rare system-level instantiation of architectural privacy irreversibility in pose-only classroom sensing through architectural memory constraints rather than post-hoc policy or obfuscation. This work focuses exclusively on sensing-layer privacy guarantees and does not evaluate pedagogical outcomes, affect inference, identity analysis, or cognitive engagement; the term ``engagement'' is used strictly to denote geometric visual-attention events (e.g., hand-raise posture and head orientation), not psychological or behavioral states.
\end{abstract}

\begin{IEEEkeywords}
Privacy-Preserving AI, Pose Estimation, GDPR-aligned data minimization, Geometric Heuristics, Edge Computing, Architectural Irreversibility, YOLOv8-Pose.
\end{IEEEkeywords}
...
Educational institutions face a dichotomy in the digitalization era: the pedagogical need for granular engagement data versus the ethical imperative to protect student privacy \cite{b1}. While identifying active participation (e.g., Q\&A frequency) provides valuable feedback loops for instructors \cite{b18}, deploying continuous video monitoring creates a surveillance environment that raises legitimate privacy concerns \cite{b2, b17}.

This conflict represents a fundamental privacy--utility trade-off. Standard computer vision solutions often prioritize utility over anonymity, capturing high-fidelity facial data that requires secure storage and handling \cite{b4}. Conversely, privacy-first sensors (like motion detectors) offer anonymity but lack the fidelity to distinguish between meaningful engagement (asking a question) and noise (stretching) \cite{b5}.

We propose an engineering solution: \textbf{Architectural Irreversibility via Pose-Only Sensing}. Instead of treating the classroom feed as video footage to be stored, we treat it as a transient stream of raw data to be vectorized and immediately destroyed. Utilizing lightweight pose estimation frameworks \cite{b6, b10}, we extract abstract skeletal landmarks (17-33 Cartesian points) in real-time. By analyzing the \textit{geometry} of these points—specifically the angular relationships between limb vectors and head orientation—we can detect participation events without ever persisting facial images or performing affect inference.

\textbf{SOTA Positioning:} To our knowledge, this represents a rare system-level instantiation of architectural privacy irreversibility in pose-only classroom sensing through architectural memory constraints rather than post-hoc policy or obfuscation. Unlike prior privacy-preserving approaches that apply obfuscation (blur/masking \cite{b3}), noise injection (differential privacy \cite{b5}), or hardware abstraction (depth sensors \cite{b9}), our contribution is the \textbf{unidirectional data flow} enforced at the sensing layer through deliberate memory destruction. By operating strictly below the inference boundary—extracting anonymous geometric events (hand raises, attention direction) without affect classification or identity linking—this architecture occupies a previously unaddressed region of the privacy-utility trade-off space, achieving 97\% event detection accuracy while structurally preventing biometric persistence, retrospective surveillance, and pixel-space reconstruction attacks (gait-based re-identification remains a residual risk; see Section IX.A).

This paper's contribution is \textbf{architectural}, not algorithmic. We do not claim novel pose estimation models or affect recognition techniques. Our novelty lies in the \textbf{systems design} that enforces privacy through memory hygiene and dimensionality-bounded irreversibility. This work constitutes an infrastructural foundation for privacy-aware learning analytics, distinct from pedagogical curriculum design.
...
\begin{enumerate}
    ...
    \item \textbf{Unidirectional Data Pipeline:} System architecture designed to overwrite video buffers in RAM instantly, ensuring no persistent Python-level references to biometric data.
\end{enumerate}
...
Early pose estimation frameworks like OpenPose \cite{b10} revolutionized human-computer interaction but required significant GPU acceleration. For edge deployment in classrooms, we utilize **lightweight pose estimation frameworks such as YOLOv8-pose \cite{b23}, MediaPipe Pose, or ViTPose (skeleton-only, no facial mesh) \cite{b6, b11, b12}**, optimized for mobile architectures. These frameworks enable 30 FPS inference on low-power devices without thermal throttling. The reference implementation uses YOLOv8n-pose for consistency with the companion biometric study (Paper 1), though the geometric heuristics are framework-agnostic.
...
Fitwi and Chen \cite{b3} proposed selective video surveillance using Gaussian blur and pixelation to obscure identity while preserving motion cues. However, this is \textit{post-hoc obfuscation}—the original high-resolution frames exist transiently in memory before blur is applied, creating a window for forensic recovery \cite{b21}. Similarly, differential privacy approaches \cite{b5} add noise to outputs but require a trusted aggregator with access to raw data.

In contrast, our approach enforces \textbf{architectural irreversibility}: raw frames are vectorized and destroyed within 33ms (one frame interval at 30 FPS), ensuring no biometric data survives beyond the Volatile Zone. This is an \textit{architectural constraint (reference-level)}, not a policy-dependent one.

\subsection{Head Pose and Attention}
Murphy-Chutorian and Trivedi \cite{b13} established that head pose is a reliable proxy for visual attention. While deep learning methods exist for this task \cite{b14}, they typically require full facial texture. Even methods robust to occlusion \cite{b15} often rely on identifiable features. Our work adapts the geometric Perspective-n-Point (PnP) approach, which solves for orientation using only sparse 2D landmarks (nose, eye corners, mouth corners), maintaining the anonymity constraint.
...
Terminology Clarification: Throughout this paper, ``engagement'' refers solely to observable geometric participation signals (pose persistence and head orientation) and does not imply affective state, motivation, cognitive load, behavioral profiling, or learning outcome inference.
...
\textbf{Crucial Distinction:} Architectural irreversibility applies strictly to biometric identity reconstruction (i.e., recovering the raw RGB pixels or facial topology). It does not preclude the longitudinal correlation of anonymous geometric behaviors (e.g., participation frequency or posture trends) when performed under lawful institutional consent and governed by the downstream policy layer.
...
\subsection{Information-Theoretic Privacy Justification (Dimensionality-Based, Intuitive)}
...
To enhance robustness beyond simple hand detection, we solve the Perspective-n-Point (PnP) problem to estimate head orientation. We map sparse cranial anchor points inferred from the pose skeleton (e.g., head centroid, neck base, and lateral cranial extrema), represented as anonymous geometric keypoints without facial texture or biometric embeddings to a standard 3D anthropometric model. No facial landmark detector, facial mesh, or identity-linked facial model is loaded or executed in the pipeline. These landmarks are derived solely as sparse geometric keypoints for orientation estimation and are not used to extract facial texture, embeddings, or identity-linked representations. Only geometric angles (yaw, pitch, roll) are computed.
...
\FOR{frame $t$ in $T$}
    ...
    \STATE $(\psi, \theta) \leftarrow SolvePnP(V.cranial_keypoints)$
...
A key concern for classroom deployment is scalability. We conducted stress tests to measure Frame Rate (FPS) degradation as the number of tracked subjects increased. The pipeline relies on cross-platform primitives and is portable to edge devices.

\begin{table}[htbp]
\caption{Scalability Stress Test - Classroom-scale deployment assumes zone-based aggregation.}
\begin{center}
\begin{tabular}{cc}
\toprule
\textbf{Subjects ($N$)} & \textbf{Average FPS} \\
\midrule
1 & 120\textsuperscript{*} \\
10 & 42 \\
30 & 18 \\
50 & 12 \\
\bottomrule
\multicolumn{2}{l}{\textsuperscript{*}\footnotesize{FPS capped by camera input rate}} \\
\multicolumn{2}{l}{\footnotesize{These measurements reflect single-camera, single-zone tracking}} \\
\multicolumn{2}{l}{\footnotesize{and should not be extrapolated to dense multi-camera fusion scenarios.}}
\end{tabular}
\end{center}
\end{table}
...
\caption{Live inference logs, showing stable 30 FPS throughput and valid coordinate extraction events.}
\label{fig:logs}
\end{figure}
...
**Scope Limitation:** In this work, gait features are treated strictly as \textbf{short-term, session-bounded motion descriptors} without identity persistence. No cross-session identifiers, re-identification keys, or longitudinal embeddings are stored or inferred. We emphasize that architectural irreversibility is designed to preclude biometric identity reconstruction, not to prohibit the correlation of anonymous behavioral trends when authorized by lawful institutional consent.
...
While this work adheres to the principles of data minimization (Article 5.1.c) and storage limitation (Article 5.1.e) as outlined in GDPR, the interpretation of biometric data processing and anonymous video analytics may vary by jurisdiction. Institutional deployment should therefore be accompanied by legal review to ensure full compliance with local privacy regulations. We do not provide legal advice; this is an engineering contribution demonstrating technical feasibility of privacy-preserving sensing. Furthermore, while the architecture mathematically prevents biometric reconstruction, it allows for behavioral correlation under lawful consent. The irreversibility boundary protects \textit{who} the student is (biometrics), while lawful data processing agreements govern \textit{what} the student does (geometric behavior).
...
\begin{itemize}
    ...
    \item \textbf{Gait Analysis:} Utilizing skeletal gait patterns to assign **session-local, non-linkable pseudonyms (e.g., "Student 5") which are regenerated each session and cannot be correlated across time**, to track participation trends over a single lecture without biometric linking.
\end{itemize}
...
\bibitem{b8} G. Casiez, N. Roussel, and D. Vogel, ``1€ Filter: A Simple Speed-based Low-pass Filter for Noisy Input in Interactive Systems,'' in \textit{Proc. CHI}, 2012, pp. 2527--2530.
...
\bibitem{b3} A. H. Fitwi and Y. Chen, ``Privacy-Preserving Selective Video Surveillance,'' in \textit{Proc. IEEE International Conference on Computer Communications and Networks (ICCCN)}, 2020, pp. 1--9.
...
\bibitem{b23} G. Jocher, A. Chaurasia, and J. Qiu, "YOLO by Ultralytics," 2023. [Online]. Available: \url{https://github.com/ultralytics/ultralytics}.
