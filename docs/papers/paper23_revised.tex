...
To resolve this challenge, this paper establishes the theoretical foundations, algorithmic design, and empirical validation of an \textit{Adaptive Risk-Driven Cascade Architecture} for edge computing. By consuming the Layer-1 Perception Risk metric $R_p \in [0, 1]$ established in \cite{kumar2026scholar22}—which unifies Dirichlet evidential uncertainty, multi-branch cross-agreement discrepancy, optical blur bounds, and landmark dispersion—the edge runtime dynamically dispatches frames along an optimal accuracy-latency Pareto frontier. Clear, unambiguous frames execute swiftly through an ultra-fast primary model ($1.264\text{ ms}$), while corrupted or ambiguous scenes trigger selective heavy verification ($14.501\text{ ms}$) or fail-closed privacy degradation.
...
Dynamic neural networks adapt their computational graphs conditioned on input difficulty \cite{han2021dynamic}. Multi-branch architectures, such as Multi-Scale Dense Networks (MSDNet) \cite{huang2017multi}, execute progressive convolutional layers and evaluate intermediate features. While dynamic architectures reduce average FLOPs, their branches share early feature representations. When an upstream sensor anomaly (e.g., optical defocus or Gaussian noise) corrupts the initial layers, the perturbation propagates through all interconnected branches, causing simultaneous classification failures across all exits \cite{hendrycks2019benchmarking}. In contrast, our architecture decouples the primary and secondary models into independent computational graphs with distinct feature extractors.
...
The adaptive cascade achieves a sustained throughput of $373.3\text{ FPS}$, representing a $5.41\times$ throughput speedup over the static heavy ensemble ($69.0\text{ FPS}$). Mean frame execution latency is $2.679\text{ ms}$. Tail latency percentiles evaluate to $P50 = 3.786\text{ ms}$, $P95 = 4.075\text{ ms}$, and $P99 = 4.556\text{ ms}$, strictly satisfying the $5.0\text{ ms}$ sub-frame SLA across all percentiles. The fast-path bypass rate is $48.0\%$, while secondary verification is triggered on $52.0\%$ of frames. Crucially, the active heavy accelerator utilization is bounded at only $8.1\%$.
...
\begin{enumerate}
    ...
    \item \textbf{Tail Latency Containment ($P99 = 4.556\text{ ms}$)}: Because average frame service time ($\mathbb{E}[S] = 2.679\text{ ms}$) provides a service rate of $\mu = 373.3\text{ FPS}$ that far exceeds nominal ingestion rates ($\lambda \le 200\text{ Hz}$), the queue buffer remains nearly empty ($Q < 2$), preventing queueing accumulation and bounding tail response time below $5.0\text{ ms}$.
\end{enumerate}
