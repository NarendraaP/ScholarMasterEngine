...
\begin{enumerate}
    \item \textbf{Constrained Pareto Optimization \& Continuum Duality}: We formulate edge multi-objective inference as a constrained optimization problem over the continuum functional space of randomized routing policies $\Pi = \{\pi: \mathcal{X} \to [0, 1]\}$. We prove that the continuum cascade formulation satisfies Slater's interior condition and exhibits a zero duality gap via Fenchel-Rockafellar strong duality.
    ...
    \item \textbf{Empirical Verification \& Graceful Degradation}: On 2,000 continuous video inferences on an edge platform, we demonstrate that our adaptive cascade achieves $373.3\text{ FPS}$ instantaneous processing service capacity ($2.679\text{ ms}$ mean service time), satisfying a strict $5.0\text{ ms}$ SLA at $P50 = 3.786\text{ ms}$, $P95 = 4.075\text{ ms}$, and $P99 = 4.556\text{ ms}$ with an active heavy core duty cycle of only $8.1\%$. We formalize a deterministic graceful degradation protocol ($Q > Q_{max}$) that prevents queue collapse under burst overloads.
\end{enumerate}
...
\begin{theorem}[Convex Continuum Duality in Edge Cascades]
...
We benchmark the Adaptive Risk-Driven Cascade on an edge-class ARM64 computing platform under fixed memory locking and persistent unified memory allocations, where both primary ($M_1$) and secondary ($M_2$) model weights remain memory-resident to avoid runtime context reload overhead. The evaluation dataset consists of 2,000 continuous video inference frames structured across five standardized sensory corruption regimes: Clean Control (Regime 1), Benign Environmental Shift (Regime 2), Physical Optical Degradation (Regime 3), Adversarial Probes (Regime 4), and Compound Adversarial-Environmental Noise (Regime 5).
...
The adaptive cascade achieves an instantaneous processing service capacity of $373.3\text{ FPS}$ (corresponding to a mean frame service time of $2.679\text{ ms}$), representing a $5.41\times$ service rate speedup over the static heavy ensemble ($69.0\text{ FPS}$). Tail latency percentiles are strictly contained at $P50 = 3.786\text{ ms}$, $P95 = 4.075\text{ ms}$, and $P99 = 4.556\text{ ms}$, well within the $5.0\text{ ms}$ sub-frame SLA. Across the benchmark, the fast-path bypass rate is $48.0\%$, while $52.0\%$ of frames undergo secondary verification, keeping active heavy core utilization capped at just $8.1\%$.
...
\bibitem{han2021dynamic} Y. Han et al., ``Dynamic neural networks: A survey,'' \emph{IEEE Trans. Pattern Anal. Mach. Intell.}, vol. 44, no. 11, pp. 7436--7456, 2022.

\bibitem{huang2017multi} G. Huang, D. Chen, T. Li, F. Wu, L. van der Maaten, and K. Q. Weinberger, ``Multi-scale dense networks for resource efficient image classification,'' in \emph{Proc. ICLR}, 2018.
...
\bibitem{geifman2019selectivenet} Y. Geifman and R. El-Yaniv, ``SelectiveNet: A deep neural network with an integrated reject option,'' in \emph{Proc. ICML}, 2019, pp. 2151--2159.
...
\bibitem{kleinrock1975queueing} L. Kleinrock, \emph{Queueing Systems, Volume 1: Theory}. New York: John Wiley \& Sons, 1975.
