...
\begin{itemize}
    ...
    \item \textbf{Reward ($R$):} The aim of $\mathcal{A}$ is to ensure that $\Phi_{comp}(\epsilon, t) = 1$ (appear compliant) while the true state $W_t \notin \Auth(\epsilon, t)$ (remains non-compliant). Let the immediate reward function $R(q, a)$ be:
    \begin{equation}
    R = \begin{cases} 
...
\begin{equation}
f_s < \frac{2 \cdot V_{max}}{L_{min}}
\end{equation}
where $L_{min} = \min_{(z_i, z_j) \in E_{adj}} d_\Z(z_i, z_j)$.
...
The proof is by contradiction. Assume that there exists some topological location $z_k \in Reach_{fwd}(\epsilon, t_0, \Delta t)$ such that $d_\Z(z_0, z_k) > V_{max} \Delta t$. Then, in order to reach it from $z_0$ during the interval of $\Delta t$, the entity would have to travel at average velocity $v = d_\Z(z_0, z_k) / \Delta t > V_{max}$. But this contradicts Axiom 2 (Kinematic Continuity). Therefore, no such $z_k$ can exist in the physically reachable set of states at a future time $t_0 + \Delta t$, which bounds the future state space of the entity, proving the theorem.
\end{proof}

\begin{proof}[Proof of Theorem 3 (Borel Measurability)]
By Theorem 1, the event calculus produces intervals of continuous state holding. Since the authorized set $\Auth(\epsilon, t)$ is defined over $\Z^*$, $\Phi_{comp}$ evaluated over any finite interval $E$ as a boolean step function has a pre-image of $\{1\}$, which is a finite union of intervals. Since intervals generate the Borel $\sigma$-algebra $\mathcal{B}(\mathbb{R})$, the set is Borel-measurable, establishing the premise for application of the Lebesgue measure $\mu$ in Definition 8 \cite{b20}.
\end{proof}
...
\begin{itemize}
    \item \textbf{State-Space Explosion \& Discrete Approximation:} While continuous spatio-temporal compliance is PSPACE-complete in the general continuous case (Theorem 6), practical real-time execution on edge appliances is achieved by discretizing time and evaluating bounded-horizon temporal predicates (e.g., polynomial-time $O(1)$ amortized PCVF debounce lookups). Characterizing the exact formal conservatism gap between continuous metric interval semantics and discrete debounced sampling remains an open problem.
...
The formal verification of spatiotemporal properties in cyber-physical systems has been addressed through several complementary traditions. Alur and Dill's timed automata \cite{b26} and hybrid automata \cite{b14} provide a foundational framework for reasoning about real-time systems with clock constraints, and their decidability results for reachability problems parallel the decidability analysis presented here. However, timed automata are typically applied to control-theoretic systems with continuous dynamics, whereas the present framework operates over discrete zone topologies with kinematic bounds.
...
Runtime verification \cite{b28} provides techniques for monitoring system executions against formal specifications, synthesized via automated safety monitors \cite{b29}. The sampling completeness analysis (Theorem 5) tackles a critical issue in the context of runtime verification, namely detectability of violations under limited observation rates. The Nyquist-style bound reported here extends previous works on monitorability \cite{b27} by giving a concrete value related to physical kinematics.

Within the CPS security problem domain, Cardenaş et al. \cite{b18} present threat models for cyber-physical systems that are useful in the development of our adversarial formulation. In particular, our MDP-based spoofing analysis (Theorem 8) builds on related work by considering an adversary that is modeled within a game-theoretic physical kinematic state and action space.

The event calculus formalization builds on the foundational work of Kowalski and Sergot \cite{b2}, temporal interval reasoning \cite{b6}, and Shanahan \cite{b19}, extending it to the spatiotemporal domain with an explicit Inertia axiom. The distributed integrity theorem draws on Lamport's foundational work on logical clocks \cite{b8}, distributed snapshots \cite{b3}, and Mattern and Fidge's vector time formalisms \cite{b13, b25} to establish causal monotonicity requirements for cross-shard entity migration. These requirements must be balanced against theoretical consistency limits in distributed environments \cite{b9}.
...
\bibitem{b12} H. Lebesgue, ``Int\'{e}grale, longueur, aire,'' \textit{Annali di Matematica Pura ed Applicata}, vol. 7, no. 1, pp. 231--235, 1902.
...
\bibitem{b14} R. Alur, C. Courcoubetis, T. A. Henzinger, and P.-H. Ho, ``Hybrid automata: An algorithmic approach to the specification and verification of hybrid systems,'' in \textit{Hybrid Systems}, Springer, 1993, pp. 209--229.
...
\bibitem{b18} A. A. Cardenas, S. Amin, and S. Sastry, ``Secure control: Towards survivable cyber-physical systems,'' in \textit{Proc. 28th International Conference on Distributed Computing Systems Workshops (ICDCSW)}, IEEE, 2008, pp. 495--500.
...
\bibitem{b13} C. J. Fidge, ``Timestamps in Message-Passing Systems That Preserve the Partial Ordering,'' in \textit{Proc. 11th Australian Computer Science Conference}, 1988, pp. 56--66.

\bibitem{b22} C. H. Papadimitriou, \textit{Computational Complexity}. Addison-Wesley, 1994.

\bibitem{b29} K. Havelund and G. Ro\c{s}u, ``Synthesizing monitors for safety properties,'' in \textit{Proc. TACAS}, Springer, 2002, pp. 342--356.
\end{thebibliography}
