...
\begin{enumerate}
    ...
    \item \textbf{Metric-Space Voronoi Discontinuity Analysis}: We prove from metric geometry that nearest-neighbor retrieval on the unit hypersphere introduces essential step jump discontinuities of magnitude $\ge 2\sin(m) \approx 0.9589$ across Voronoi facets, formally explaining why continuous global Lipschitz verification fails in deep metric retrieval and proving the necessity of certified domain partitioning ($\mathcal{X}_{cert} \cup \mathcal{X}_{quar}$).
    ...
    \item \textbf{Empirical Macro Containment \& Compounding Telemetry}: Across five standardized corruption levels ($0\%$, $5\%$, $10\%$, $15\%$, $20\%$), we demonstrate that unprotected pipelines amplify errors from $21.33\%$ in Layer 2 to $38.90\%$ in Layer 4 (peak local $\text{EAF} = 1.4220$). Under Layer-1 Perception Integrity gating, uncertified frames are safely quarantined, achieving zero downstream error propagation ($\text{EAF} = 0.0000$) on the admitted processing stream.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Layer 1 (Perception Integrity)}: Ingests raw multi-modal sensory observations $\mathbf{x} = \{I, \mathbf{k}, \mathbf{a}\}$ \cite{khaleghi2013multisensor}, evaluates evidential uncertainty \cite{guo2017calibration}, multi-branch disagreement, Laplacian blur, and temporal consistency, computing the composite risk $R_p(\mathbf{x}) \in [0, 1]$ \cite{kumar2026scholar22}. It emits a validated sensory payload $\mathcal{P}(\mathbf{x})$ if $R_p(\mathbf{x}) \le \tau_{risk} = 0.70$, or triggers immediate fail-closed quarantine ($\bot$) otherwise.
...
\bibitem{weng2018towards} L. Weng, H. Zhang, H. Chen, Z. Song, C.-J. Hsieh, L. Daniel, D. Boning, and I. Dhillon, ``Towards fast computation of certified robustness for ReLU networks,'' in \emph{Proc. International Conference on Machine Learning (ICML)}, 2018, pp. 5276--5285.
